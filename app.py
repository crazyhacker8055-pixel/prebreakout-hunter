...
def main():
    st.title('🎯 Pre-Breakout Hunter V8.2')
...
V8 deliberately rewards *readiness* rather than simply giving points for a good-looking trend. Results are **watchlist candidates, not automatic buy signals**.''')
    ...
    st.markdown('### 📈 V8.2 Long Walk-Forward Trade Validation')
    st.caption('Validation is independent of the normal scan. It uses only information available on each historical setup date. Breakout = first close above setup pivot. Entry = next-day open. Exit = next-day open after a close below 220 SMA or -15% price risk, with a time cap.')
    with st.expander('Run V8.2 historical trade validation', expanded=bool(st.session_state.validation_status)):
        setup_days=st.slider('Historical setup days',100,400,200,50,key='val_setup_days')
        bw=st.slider('Breakout window (days)',5,20,10,key='val_bw')
        td=st.slider('Maximum observation / trade days',15,40,30,key='val_td')
        if st.button('🧪 RUN V8.2 TRADE VALIDATION',width='stretch'):
            st.session_state.validation_result=pd.DataFrame()
            st.session_state.validation_status='Running'
            st.info('Validation is independent of the normal scanner. Historical data downloads in independently cached batches; completed batches can be reused on reruns.')
            dlp=st.progress(0, text='Downloading historical data…')
            hist_frames=download_batches_resumable(tuple(symbols),period_days=1100,batch_size=50,progress=dlp)
            dlp.empty()
            ...
            st.session_state.validation_status='Completed'
            if bt.empty:
                st.warning('No historical V8 setups were found in this sample. Try increasing Historical setup days or slightly widening the scanner gates only for research; do not change rules solely to improve returns.')
            else:
        ...
        if not bt.empty:
            ...
            if not trades.empty:
                ...
                st.download_button('⬇️ Download V8.2 validation CSV',bt.to_csv(index=False).encode(),'v8_2_validation.csv','text/csv')
