...
st.set_page_config(page_title='Pre-Breakout Hunter V9.3.2', page_icon='🎯', layout='wide')
...
# V9.3.2: NIFTY 500 -> proven LTPC WebSocket + V3 REST 1-minute OHLC polling.
...
def _poll_upstox_i1_ohlc_loop(mapped):
    ...
    while True:
        ...
        try:
            ...
            # Upstox documents V3 OHLC `data` as an object keyed by exchange:symbol,
            # but keep this parser defensive because proxies/SDK wrappers can return
            # a list-shaped container.
            if isinstance(raw_data, dict):
                ...
                for idx, item in enumerate(raw_data):
                    if isinstance(item, dict):
                        key = item.get("instrument_token") or item.get("instrument_key") or item.get("symbol") or str(idx)
                        data_items.append((str(key), item))
            else:
                data_items = []

            candles = {}
            for api_key, item in data_items:
                if not isinstance(item, dict):
                    continue
                ikey = str(item.get("instrument_token") or item.get("instrument_key") or str(api_key).replace(":", "|"))
                sym = reverse.get(ikey)
                if not sym:
                    # Some responses key the item by NSE_EQ:SYMBOL rather than the
                    # pipe-form instrument token. Recover the symbol from that key.
                    candidate = str(api_key).split(":", 1)[-1]
                    if candidate in mapped:
                        sym = candidate
                if not sym:
                    continue

                def normalize_candle(value):
                    if isinstance(value, dict):
                        return value
                    if isinstance(value, list):
                        # Be tolerant of a single-element list wrapper.
                        for v in value:
                            if isinstance(v, dict):
                                return v
                    return {}

                live = normalize_candle(item.get("live_ohlc"))
                if not live:
                    live = normalize_candle(item.get("ohlc"))
                prev = normalize_candle(item.get("prev_ohlc"))
...
def _start_upstox_nifty500_websocket(symbols):
    ...
    V9.3.2 intentionally keeps the WebSocket on LTPC because the same
    ...
    def worker():
        ...
        try:
            ...
            with UPSTOX_WS_LOCK:
                ...
                # Do not stop the hybrid engine here; REST OHLC polling owns its
                # own lifecycle and can still supply 1-minute data.
...
if hasattr(st, "fragment"):
    ...
    def render_upstox_nifty500_panel():
        ...
        elif running and ohlc_last_poll:
            st.info("🟡 Hybrid engine running — WebSocket unavailable, V3 REST OHLC is active")
        elif running:
            st.info("🟡 Connecting NIFTY 500 live engine…")
        else:
            st.warning("⚪ NIFTY 500 stream is stopped")
        a,b,c,d = st.columns(4)
        connection_label = "LIVE" if connected else ("HYBRID" if running and ohlc_last_poll else ("CONNECTING" if running else "STOPPED"))
        a.metric("Connection", connection_label)
...
def upstox_nifty500_stream_test(symbols):
    ...
    st.caption(
        "V9.3.2 uses the official NSE Indices NIFTY 500 constituent file first, then Upstox NSE_EQ mapping with instrument-search repair for valid equities. "
...
def main():
    st.title('🎯 Pre-Breakout Hunter V9.3.2')
