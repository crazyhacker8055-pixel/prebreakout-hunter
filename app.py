...
st.set_page_config(page_title='Pre-Breakout Hunter V9.1', page_icon='🎯', layout='wide')
...
# V9.1: small, persistent Upstox WebSocket V3 test state.
# This is deliberately global so Streamlit reruns do not destroy the background
# WebSocket object. It is NOT used for order placement.
UPSTOX_WS_STATE = {
    "streamer": None,
    "thread": None,
    "running": False,
...
UPSTOX_WS_INSTRUMENTS = {
    "NIFTY 50": "NSE_INDEX|Nifty 50",
    "NHPC": "NSE_EQ|INE848E01016",
    "RELIANCE": "NSE_EQ|INE002A01018",
    "TEST EQUITY A": "NSE_EQ|INE020B01018",
    "TEST EQUITY B": "NSE_EQ|INE467B01029",
}
...
def _find_ltpc(obj):
    """Recursively find the first LTPC dict in an SDK-decoded feed message."""
    if isinstance(obj, dict):
...
def _start_upstox_websocket():
    ...
    def worker():
        ...
        try:
            ...
            keys = list(UPSTOX_WS_INSTRUMENTS.values())
            streamer = upstox_client.MarketDataStreamerV3(api_client, keys, "full")
            with UPSTOX_WS_LOCK:
                UPSTOX_WS_STATE["streamer"] = streamer
            ...
            def on_message(message):
                updates = _extract_live_updates(message)
                with UPSTOX_WS_LOCK:
            ...
            # Keep the test stable; the user can reconnect manually.
            try:
    ...
    thread = threading.Thread(target=worker, name="upstox-v3-stream", daemon=True)
    with UPSTOX_WS_LOCK:
...
if hasattr(st, "fragment"):
    ...
    def render_upstox_stream_panel():
        with UPSTOX_WS_LOCK:
        ...
        if connected:
            st.success("🟢 LIVE — Upstox WebSocket V3 connected")
        elif running:
            st.info("🟡 Connecting to Upstox WebSocket V3…")
        else:
            st.warning("⚪ WebSocket is stopped")

        a, b, c = st.columns(3)
        a.metric("Connection", "LIVE" if connected else ("CONNECTING" if running else "STOPPED"))
        b.metric("Instruments Updating", str(len(updates)))
        c.metric("Last Feed", last_message_at or "—")
        if opened_at:
            st.caption(f"Connected at {opened_at} server time.")
        if error:
        ...
        if updates:
            ...
            df = pd.DataFrame(rows)
            if "Change %" in df:
                df["Change %"] = df["Change %"].apply(lambda x: round(float(x), 2) if pd.notna(x) else np.nan)
            st.dataframe(df, use_container_width=True, hide_index=True)
        else:
            st.caption("Waiting for the first market-data snapshot…")


def upstox_websocket_test():
    st.markdown("## 📡 Upstox WebSocket V3 Streaming Test")
    st.caption(
        "V9.1 test only: verifies persistent real-time streaming with a small basket. "
        "Read-only — no order API is used."
    )
    st.write("**Test basket:** NIFTY 50 • NHPC • RELIANCE • 2 Upstox documentation test equities")
    c1, c2 = st.columns(2)
    if c1.button("▶️ START LIVE STREAM", width="stretch"):
        ok, msg = _start_upstox_websocket()
        if ok:
            st.toast("Upstox WebSocket starting…")
        else:
            st.warning(f"Could not start stream: {msg}")
    if c2.button("⏹️ STOP STREAM", width="stretch"):
        _stop_upstox_websocket()
        st.toast("Upstox WebSocket stopped")
    render_upstox_stream_panel()
    st.caption("Upstox V3 sends an initial market-status message, then a snapshot and subsequent live updates. This test intentionally uses only a few instruments before we scale to NIFTY 500.")

def main():
    st.title('🎯 Pre-Breakout Hunter V9.1')
    upstox_connection_test()
    st.divider()
    upstox_websocket_test()
    st.divider()
