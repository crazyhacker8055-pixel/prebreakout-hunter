...
st.set_page_config(page_title='Pre-Breakout Hunter V8.7', page_icon='🎯', layout='wide')
...
def market_dashboard(frames, symbols, sector_map):
    """Build current-market breadth tables from the same daily OHLCV data used by the scanner."""
    rows=[]
    for sym in symbols:
        df=frames.get(f'{sym}.NS',pd.DataFrame())
        if len(df)<21: continue
        try:
            c=float(df.close.iloc[-1]); prev=float(df.close.iloc[-2])
            v=float(df.volume.iloc[-1]); v20=float(df.volume.iloc[-21:-1].mean())
            chg=(c/prev-1)*100 if prev else np.nan
            vs=(v/v20-1)*100 if v20 else np.nan
            rows.append({'Stock':sym,'Close':c,'Change %':chg,'Volume':v,'Volume Surge %':vs,
                         'Sector':sector_map.get(sym,'Other / Unclassified')})
        except Exception:
            continue
    mkt=pd.DataFrame(rows)
    if mkt.empty: return mkt,pd.DataFrame(),pd.DataFrame()
    gainers=mkt.sort_values('Change %',ascending=False).head(15).copy()
    vg=mkt.sort_values('Volume Surge %',ascending=False).head(15).copy()
    sec=mkt.assign(Advance=mkt['Change %']>0,Decline=mkt['Change %']<0)
    sector=sec.groupby('Sector',dropna=False).agg(Stocks=('Stock','count'),Advances=('Advance','sum'),Declines=('Decline','sum')).reset_index()
    sector['Unchanged']=sector['Stocks']-sector['Advances']-sector['Declines']
    sector['Advance %']=(sector['Advances']/sector['Stocks']*100).round(1)
    sector['Net Advance']=sector['Advances']-sector['Declines']
    sector=sector.sort_values(['Advance %','Net Advance'],ascending=[False,False]).reset_index(drop=True)
    for d in (gainers,vg):
        d['Close']=d['Close'].round(2); d['Change %']=d['Change %'].round(2); d['Volume Surge %']=d['Volume Surge %'].round(1)
    return mkt,gainers,vg,sector
...
def main():
    st.title('🎯 Pre-Breakout Hunter V8.7')
    st.caption('Selective NSE scanner + live market breadth dashboard — resistance pressure + progressive compression + higher lows + improving relative strength. No broker API required.')
...
V8 deliberately rewards *readiness* rather than simply giving points for a good-looking trend. Results are **watchlist candidates, not automatic buy signals**.''')
    ...
    if scan_btn:
        ...
        st.session_state.market_volume_gainers=md[2]
        st.session_state.market_sector=md[3]
    ...
    if st.session_state.scan_ran and not st.session_state.market_data.empty:
        ...
        st.caption('Built from the same current daily OHLCV data used by the scanner. Volume gainers are ranked by today’s volume versus the previous 20-session average.')
        md=st.session_state.market_data
        adv=int((md['Change %']>0).sum()); dec=int((md['Change %']<0).sum()); unch=int((md['Change %']==0).sum())
        a,b,c,d=st.columns(4)
        a.metric('Stocks advancing',adv)
        b.metric('Stocks declining',dec)
        c.metric('Unchanged',unch)
        d.metric('Advance / Decline',f'{adv}:{dec}')
        t1,t2,t3=st.tabs(['🚀 Top Gainers','🔊 Volume Gainers','🗺️ Sector Advance Map'])
        with t1:
            g=st.session_state.market_gainers.copy()
            if not g.empty:
                st.dataframe(g[['Stock','Close','Change %','Volume','Sector']],width='stretch',hide_index=True)
        ...
        with t2:
            ...
            if not vg.empty:
                st.dataframe(vg[['Stock','Close','Change %','Volume','Volume Surge %','Sector']],width='stretch',hide_index=True)
        ...
        with t3:
            ...
            if not sec.empty:
                # Compact visual heat-map style tiles; each tile is one sector.
                tiles=[]
                for _,r in sec.iterrows():
                    pct=float(r['Advance %']); bg='#1b7f3a' if pct>=60 else ('#7f8c32' if pct>=40 else '#a33a3a')
                    tiles.append(f"<div style='display:inline-block;vertical-align:top;width:30%;min-width:180px;margin:5px;padding:12px;border-radius:10px;background:{bg};color:white'><b>{r['Sector']}</b><br>Advance {pct:.1f}%<br>↑ {int(r['Advances'])} &nbsp; ↓ {int(r['Declines'])}<br>{int(r['Stocks'])} stocks</div>")
                st.markdown(''.join(tiles),unsafe_allow_html=True)
                st.dataframe(sec[['Sector','Stocks','Advances','Declines','Unchanged','Advance %','Net Advance']],width='stretch',hide_index=True)
