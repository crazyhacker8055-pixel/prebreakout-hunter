...
UPSTOX_WS_STATE = {
    ...
    "seed_thread": None, "seed_running": False, "seed_done": False, "seed_ok": 0, "seed_total": 0, "seed_error": "",
}
...
def _seed_upstox_intraday_history(mapped):
    """Seed today's 1-minute history, then the live poller keeps it current."""
    try:
        token = str(st.secrets.get("UPSTOX_ACCESS_TOKEN", "")).strip()
    except Exception:
        token = ""
    if not token or not mapped:
        with UPSTOX_WS_LOCK:
            UPSTOX_WS_STATE["seed_error"] = "Missing token or mapped instruments for intraday history seed."
            UPSTOX_WS_STATE["seed_running"] = False
        return
    items=list(mapped.items()); total=len(items); ok=0; errors=0
    headers={"Accept":"application/json","Authorization":f"Bearer {token}"}
    def fetch_one(item):
        symbol, ikey=item
        try:
            url="https://api.upstox.com/v3/historical-candle/intraday/{}/minutes/1".format(requests.utils.quote(str(ikey), safe=""))
            r=requests.get(url,headers=headers,timeout=15); r.raise_for_status()
            payload=r.json(); candles=((payload.get("data") or {}).get("candles") or []) if isinstance(payload,dict) else []
            rows=[]
            for c in candles:
                if isinstance(c,(list,tuple)) and len(c)>=6:
                    rows.append({"Symbol":symbol,"Instrument Key":ikey,"1m Timestamp":c[0],"1m Open":c[1],"1m High":c[2],"1m Low":c[3],"1m Close":c[4],"1m Volume":c[5]})
            rows.sort(key=lambda x:str(x["1m Timestamp"]))
            return symbol,rows[-120:],None
        except Exception as exc:
            return symbol,[],f"{type(exc).__name__}: {exc}"
    # Seed sequentially to protect Streamlit Cloud and stay comfortably below
    # Upstox's standard API limits. The previous 12-worker burst could overload
    # the app while 500 historical requests were being launched at once.
    for idx, item in enumerate(items, start=1):
        symbol,rows,err=fetch_one(item)
        if rows:
            _update_candle_history({symbol:rows}); ok+=1
        else:
            errors+=1
        with UPSTOX_WS_LOCK:
            UPSTOX_WS_STATE["seed_ok"]=ok; UPSTOX_WS_STATE["seed_total"]=total
            if err and not UPSTOX_WS_STATE.get("seed_error"):
                UPSTOX_WS_STATE["seed_error"]=f"{symbol}: {err}"
        # Small pacing gap prevents request bursts and leaves headroom for the
        # five 100-key live OHLC requests running in parallel.
        time.sleep(0.10)
    with UPSTOX_WS_LOCK:
        UPSTOX_WS_STATE["seed_running"]=False; UPSTOX_WS_STATE["seed_done"]=True
        UPSTOX_WS_STATE["seed_ok"]=ok; UPSTOX_WS_STATE["seed_total"]=total
        if errors and not UPSTOX_WS_STATE.get("seed_error"):
            UPSTOX_WS_STATE["seed_error"]=f"{errors} symbols returned no intraday candles."


def _poll_upstox_i1_ohlc_loop(mapped):
    """Near-live 1-minute OHLCV polling, split into resilient batches."""
    try:
        token = str(st.secrets.get("UPSTOX_ACCESS_TOKEN", "")).strip()
    except Exception:
        token = ""
    ...
    while True:
        ...
        try:
            ...
            with UPSTOX_WS_LOCK:
                ...
                if errors:
                    UPSTOX_WS_STATE["ohlc_error"] = f"I1 poll partial: {successful_batches}/{(len(keys)+batch_size-1)//batch_size} batches; " + " | ".join(errors)
                else:
...
def _start_upstox_nifty500_websocket(symbols):
    ...
    "ohlc_thread": None, "ohlc_running": False, "ohlc_error": "", "ohlc_last_poll": None, "ohlc_symbols": 0, "ohlc_candles": 0, "rest_live": False,
            "seed_thread": None, "seed_running": False, "seed_done": False, "seed_ok": 0, "seed_total": 0, "seed_error": "", "mapped": mapped,
    ...
    seed_thread = threading.Thread(target=_seed_upstox_intraday_history, args=(mapped,), name="upstox-v3-intraday-seed", daemon=True)
    with UPSTOX_WS_LOCK:
        UPSTOX_WS_STATE["thread"] = thread
        UPSTOX_WS_STATE["ohlc_thread"] = ohlc_thread
        UPSTOX_WS_STATE["seed_thread"] = seed_thread
    ...
    ohlc_thread.start(); seed_thread.start()
...
if hasattr(st, "fragment"):
    ...
    def render_upstox_nifty500_panel():
        ...
        if seed_running:
            st.info(f"⏳ Seeding today's 1-minute history: {seed_ok}/{seed_total} stocks loaded. Stable mode uses one request at a time to protect the server and Upstox rate limits.")
        elif seed_done:
            st.success(f"✅ Intraday history seeded: {seed_ok}/{seed_total} stocks. Radar has real 1-minute context.")
        if seed_error:
...
def upstox_nifty500_stream_test(symbols):
    ...
    st.caption(
        "V9.4 uses the official NSE Indices NIFTY 500 constituent file first, then Upstox NSE_EQ mapping with instrument-search repair for valid equities. "
    ...
    st.caption("Confirmation layer from seeded + live 1-minute candles. It does not replace the strict daily V8 setup; Daily Setup = YES means the stock also passed the saved daily scan.")
    with UPSTOX_WS_LOCK:
...
def main():
    st.title('🎯 Pre-Breakout Hunter V9.4.2')
