...
def _start_upstox_nifty500_websocket(symbols):
    ...
    with UPSTOX_WS_LOCK:
        ...
        UPSTOX_WS_STATE["seed_error"] = ""
    ohlc_thread.start()
    # V9.5: do not seed 500 stocks at startup. The focused seed is triggered
...
def upstox_nifty500_stream_test(symbols):
    ...
    st.caption(
        "V9.5 uses the official NSE Indices NIFTY 500 constituent file first, then Upstox NSE_EQ mapping with instrument-search repair for valid equities. "
    ...
    st.caption("Confirmation layer for the focused daily watchlist. V9.5 does not seed all 500 stocks; it seeds strict daily candidates + strongest near-misses, then confirms them with live 1-minute structure.")
    with UPSTOX_WS_LOCK:
...
def main():
    st.title('🎯 Pre-Breakout Hunter V9.5')
...
V8 deliberately rewards *readiness* rather than simply giving points for a good-looking trend. Results are **watchlist candidates, not automatic buy signals**.''')
    ...
    if scan_btn:
        ...
        # V9.5: after the daily scan, create the focused intraday radar universe.
