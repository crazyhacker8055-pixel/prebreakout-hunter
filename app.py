...
st.set_page_config(page_title="Pre-Breakout Hunter V21", page_icon="🎯", layout="wide")
...
def live_regrade(structure_df, frames, live_map, mapping, min_score=84, near_score=72, now=None, near_limit=10):
    ...
    # Live score starts from the full historical structure score. Unlike V20,
    # ALL stocks reach this stage; there is no daily-score pre-filter.
    out["Live Score"]=out["Score"].fillna(0).astype(float)
    dist=out["Live To Pivot %"]
    out.loc[(dist>=0)&(dist<=5),"Live Score"] += np.clip(8-(dist*1.6),0,8)
    out.loc[(dist>=0)&(dist<=1.5),"Live Score"] += 6
    out.loc[out["Live High"]>=out["Pivot"],"Live Score"] += 8
    out.loc[out["Minute High"]>=out["Pivot"]*0.997,"Live Score"] += 3
    out.loc[out["Live Position %"]>=75,"Live Score"] += 2
    out.loc[out["Live Fresh"]==False,"Live Score"] -= 20
    out["Live Score"]=out["Live Score"].clip(0,100).round().astype(int)

    # Strict breakout: strong structure + fresh price at/above prior resistance.
    strict_structure=(out["Score"]>=min_score)
    breakout=(out["LTP"]>=out["Pivot"]) & strict_structure & (out["Live Fresh"]==True)
    ready=(out["LTP"]<out["Pivot"]) & (out["Live To Pivot %"]>=0) & (out["Live To Pivot %"]<=3.5) & (out["Live Score"]>=min_score) & (out["Live Fresh"]==True)
    # Near-miss is intentionally broader: closest live names even when the
    # daily score has not crossed the READY threshold yet.
    near=((out["LTP"]<out["Pivot"]) & (out["Live To Pivot %"]>=0) & (out["Live To Pivot %"]<=5.0) & (out["Live Score"]>=near_score))
    ...
    # Breakout first, then readiness, then distance to resistance.
    out=out.sort_values(["Live Score","Live To Pivot %"],ascending=[False,True])
    return out
...
def main():
    ...
    st.caption("NIFTY 500 → full-universe live snapshot → daily structure context → 1-minute pressure → breakout/ready/near-miss")
    if not token():
        ...
        if idx < total:
            ...
            try:
                ...
                if live_df.empty:
                    ...
                    # Always surface the closest live candidates if the strict
                    # near-miss gate produces fewer than five names. This is a
                    # ranked WATCHLIST, not a false breakout signal.
                    if len(near_df)<5:
                        watch=live_df[(live_df["Stage"]=="WATCH") & (live_df["LTP"]<live_df["Pivot"]) & (live_df["Live To Pivot %"]>=0) & (live_df["Live To Pivot %"]<=7)].copy()
                        watch=watch.sort_values(["Live Score","Live To Pivot %"],ascending=[False,True]).head(5-len(near_df))
                        if not watch.empty:
                            watch["Stage"]="NEAR-MISS NOW"; watch["Live Status"]="🟡 NEAR-MISS NOW"
                            near_df=pd.concat([near_df,watch],ignore_index=True).drop_duplicates("Stock")
                            near_df=near_df.sort_values(["Live Score","Live To Pivot %"],ascending=[False,True])
...
""")
    st.caption("Pre-Breakout Hunter V21 • LIVE-FIRST full-universe engine • Upstox Analytics Token • no Yahoo • no order/trading API")
