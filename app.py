...
st.set_page_config(page_title="Pre-Breakout Hunter V10.0", page_icon="🎯", layout="wide")
...
def get_nifty500_constituents():
    ...
    for url in NIFTY500_URLS:
        try:
            r = requests.get(url, headers={"User-Agent": "Mozilla/5.0 Pre-Breakout-Hunter-V10"}, timeout=20)
            r.raise_for_status()
            ...
            out = pd.DataFrame({"Symbol": t[sym_col].astype(str).str.strip()})
            if sector_col:
                out["Sector"] = t[sector_col].astype(str).str.strip()
            else:
                out["Sector"] = "Other / Unclassified"
...
def get_upstox_equity_master():
    r = requests.get(UPSTOX_INSTRUMENTS_URL, headers={"User-Agent": "Mozilla/5.0 Pre-Breakout-Hunter-V10"}, timeout=30)
    r.raise_for_status()
    ...
    for x in data:
        ...
        if str(x.get("segment", "")) != "NSE_EQ":
            continue
        if str(x.get("instrument_type", "")).upper() not in {"EQ", "EQUITY"}:
            continue
...
def build_mapping(constituents, master):
    ...
    return c.merge(m, on="Symbol", how="left")
...
def fetch_history_one(symbol, instrument_key, start_date, end_date):
    ...
    try:
        ...
        if len(df) < MIN_BARS:
            return symbol, df, f"Only {len(df)} daily bars"
        return symbol, df, ""
    except Exception as e:
        return symbol, pd.DataFrame(), str(e)[:160]


def fetch_histories(mapping, progress=None):
    ...
    errors = {}
    total = len(valid)
    # Upstox standard API limit is 500/min. Eight workers are unnecessary and can burst.
    # Five workers with a small global pacing keeps the scanner under the documented ceiling.
    ...
    def paced_fetch(item):
        ...
        return fetch_history_one(symbol, key, start.isoformat(), end.isoformat())

    with ThreadPoolExecutor(max_workers=5) as ex:
        futs = [ex.submit(paced_fetch, x) for x in jobs]
...
def fetch_nifty_history():
    # Current Upstox instrument key for Nifty 50.
    end = datetime.now(timezone.utc).astimezone().date()
    start = end - timedelta(days=HISTORY_DAYS)
    url = f"https://api.upstox.com/v3/historical-candle/{quote('NSE_INDEX|Nifty 50', safe='')}/days/1/{end.isoformat()}/{start.isoformat()}"
    r = requests.get(url, headers=headers(), timeout=12)
    if r.status_code != 200:
        raise RuntimeError(f"NIFTY 50 history HTTP {r.status_code}")
    df = parse_candles(r.json())
    if len(df) < MIN_BARS:
        raise RuntimeError(f"NIFTY 50 history returned only {len(df)} daily bars")
    return df["close"]
...
def fetch_live_ltp(keys):
    rows = {}
    errors = []
    # Keep a conservative chunk size even though the endpoint supports multiple keys.
...
def compute_metrics(d, nifty):
    ...
    # 52-week position and recent breakout distance.
    high252 = h.iloc[-252:].max()
    low252 = l.iloc[-252:].min()
    pos52 = (close-low252)/(high252-low252)*100 if high252 > low252 else np.nan
    high20 = h.iloc[-20:].max()
    return locals()
...
def evaluate(d, nifty, cfg):
    ...
    if hard and score >= cfg["min_score"]:
...
def run_scan(mapping, cfg, progress_box):
    ...
    frames, errors, start, end = fetch_histories(mapping, progress)
    progress_bar.progress(0.98, text="Downloading NIFTY 50 benchmark…")
    nifty = fetch_nifty_history()
...
def main():
    ...
    st.caption("Clean architecture: Upstox read-only market data • completed daily candles for the pattern • live LTP only for current market position")
    ...
    try:
        ...
        mapping = build_mapping(constituents, master)
    except Exception as e:
    ...
    # Persistent scan state is intentionally small and contains only daily data.
    ...
    if scan_button:
        ...
        try:
            ...
            st.session_state.v10_result = {
                "results": result_df, "near": near_df, "frames": frames, "errors": errors,
                "nifty": nifty, "mapping": mapping, "start": start, "end": end,
                "fetched_at": datetime.now(timezone.utc).astimezone(), "cfg": cfg,
            }
        except Exception as e:
            progress_box.empty()
    ...
    if state is None:
        ...
        st.warning("Run the scan. V10 uses completed daily candles for the setup, so it does not manufacture 1-minute history or stale intraday candles.")
        return
    ...
    # Live LTP is deliberately a separate, single-purpose call. It never affects historical setup calculations.
    live_by_key, live_errors = fetch_live_ltp(mapped.instrument_key.astype(str).tolist())
    rows = []
    ...
    a.metric("Live quotes", f"{len(live_df)}/{len(mapped)}")
    b.metric("Setup candidates", str(len(results)))
    c.metric("Near-miss watchlist", str(len(near)))
    d.metric("Daily data fetched", f"{len(frames)}/{len(mapped)}")
    st.caption(f"Daily history fetched: {fetched_at.strftime('%Y-%m-%d %H:%M:%S %Z')} • live LTP fetched immediately after scan")
    if live_errors:
        st.warning("Some live LTP chunks failed: " + "; ".join(live_errors[:3]))

    # Sector strength from live LTP, not stale historical prices.
    ...
    if not results.empty:
        ...
        st.success("These are watchlist candidates, not automatic buy signals. A daily closing breakout above the pivot is still required.")
    else:
        st.warning("No A/A+ pre-breakout candidates met every hard gate today. V10 intentionally does not loosen rules to force a trade.")

    if not near.empty:
        st.subheader("🟡 Near-miss watchlist")
        st.caption("These are structurally close, but at least one hard gate is not satisfied. They are not buy signals.")
        st.dataframe(near, width="stretch", hide_index=True)

    with st.expander("🩺 Data health / exact failures"):
        st.write(f"Successful daily histories: {len(frames)} / {len(mapped)}")
        st.write(f"History window requested: {state['start']} → {state['end']}")
        if errors:
        ...
        st.caption("V10 never labels a failed or missing history as a valid setup.")

    with st.expander("📐 Exact V10 setup rules"):
        st.markdown("""
- Strong trend: Close > SMA50 > SMA150 > SMA200, rising SMA200, and Close > EMA20.
- Progressive contraction: 5D range < 10D range < 20D range and each below the configured maximum.
- Volatility contraction: ATR5/ATR20 and 5D/20D volume ratios must be below the configured limits.
- Structure: higher lows, repeated resistance tests, and price 0.25–3% below the prior 20-session pivot by default.
- No recent breakout: the last five completed closes must remain below their prior 20-session highs.
- Relative strength: stock must outperform NIFTY 50 over the recent 20-session window.
- Distribution filter: average down-day volume must not materially dominate up-day volume.
- Final score must meet the minimum score, with all hard gates passing.
""")

    st.caption("V10 architecture: Upstox read-only Analytics Token → NIFTY 500 mapping → Upstox daily history → deterministic scanner → one fresh Upstox LTP snapshot. No trading API is used.")
