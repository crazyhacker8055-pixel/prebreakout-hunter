...
def evaluate(df, nifty, cfg):
    """Evaluate a stock and return both strict result and diagnostic near-miss data."""
    ...
    # Component score is deliberately continuous enough to rank near-misses.
    score = 0
    ...
    score += 10 if r5 <= 4.0 else (5 if r5 <= cfg['range5'] else 0)
    score += 10 if r10 <= 7.0 else (5 if r10 <= cfg['range10'] else 0)
    score += 10 if atr_ratio <= 0.65 else (5 if atr_ratio <= cfg['atr_ratio'] else 0)
    score += 10 if vol_ratio <= 0.60 else (5 if vol_ratio <= cfg['vol_ratio'] else 0)
    score += 10 if cfg['pivot_min'] <= distance <= 2.0 else (5 if cfg['pivot_min'] <= distance <= cfg['pivot_max'] else 0)
    score += 5 if rs >= 3 else 0
    score += 5 if rs >= 6 else 0

    reasons = []
    if not (x.close > x.sma50 > x.sma150 > x.sma200 and x.sma200 > sma200_20 and x.close > x.ema20):
        reasons.append('Trend not strong enough')
    if not (r5 <= cfg['range5'] and r10 <= cfg['range10'] and r20 <= cfg['range20']):
        reasons.append('Base ranges too wide')
    if not (r5 < r10 < r20):
        reasons.append('Range is not contracting 20D → 10D → 5D')
    if not (np.isfinite(atr_ratio) and atr_ratio <= cfg['atr_ratio']):
        reasons.append('ATR contraction failed')
    if not (np.isfinite(vol_ratio) and vol_ratio <= cfg['vol_ratio']):
        reasons.append('Volume dry-up failed')
    if not (cfg['pivot_min'] <= distance <= cfg['pivot_max'] and x.close < pivot):
        if x.close >= pivot:
            reasons.append('Already broke pivot')
        elif distance > cfg['pivot_max']:
            reasons.append('Too far below pivot')
        elif distance < cfg['pivot_min']:
            reasons.append('Too close to pivot')
    if recent_breakout:
        reasons.append('Recent breakout detected')
    if not higher_low:
        reasons.append('Higher-low structure failed')
    if distribution:
        reasons.append('Distribution pressure detected')
    if rs < 0:
        reasons.append('Relative strength below NIFTY')
    if score < cfg['min_score']:
        reasons.append(f'Score below {cfg["min_score"]}')
    ...
    result = {
        ...
        'Why it missed': 'PASS — all strict filters' if strict else ' • '.join(reasons[:3]),
        '_strict': strict,
    }
    return result
