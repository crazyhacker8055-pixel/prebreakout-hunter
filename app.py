...
st.set_page_config(page_title='Pre-Breakout Hunter V9.5.4', page_icon='🎯', layout='wide')
...
def _poll_upstox_i1_ohlc_loop(mapped):
    """Rate-limit-safe live engine: LTP for NIFTY 500 + I1 OHLC only for radar stocks.

    The full universe needs only current price/volume. Detailed 1-minute OHLCV is
    intentionally restricted to the focused daily candidates/near-misses so the
    Upstox API is not hammered with five 100-key OHLC requests every few seconds.
    """
    try:
        token = str(st.secrets.get("UPSTOX_ACCESS_TOKEN", "")).strip()
    except Exception:
    ...
    while True:
        ...
        # ---- Full-universe current snapshot: ONE LTP request for up to 500 keys. ----
        if now >= next_ltp:
            try:
                r = requests.get(
                ...
                if isinstance(data, dict):
                    with UPSTOX_WS_LOCK:
                        ...
                        for api_key, obj in data.items():
                            ...
                            # Keep any existing 1-minute candle fields.
                ...
                if getattr(getattr(exc, "response", None), "status_code", None) == 429:
                    ltp_backoff = min(120, max(30, (ltp_backoff or 30) * 2))
                next_ltp = time.time() + (ltp_backoff or 30)

        # ---- Detailed I1 OHLCV only for the focused radar watchlist. ----
        if watchlist and now >= next_i1:
            focused_keys = [mapped[s] for s in watchlist if s in mapped]
            try:
                total_rows = 0
                total_candles = 0
                successful_batches = 0
                batch_size = 100
                for offset in range(0, len(focused_keys), batch_size):
                    batch = focused_keys[offset:offset + batch_size]
                    r = requests.get(
                ...
                with UPSTOX_WS_LOCK:
                    ...
                    if not errors:
                        UPSTOX_WS_STATE["ohlc_error"] = ""
                i1_backoff = 0
                next_i1 = time.time() + i1_interval
            except Exception as exc:
                errors.append(f"I1: {type(exc).__name__}: {exc}")
                if getattr(getattr(exc, "response", None), "status_code", None) == 429:
                    i1_backoff = min(120, max(30, (i1_backoff or 30) * 2))
                next_i1 = time.time() + (i1_backoff or 30)
...
def upstox_nifty500_stream_test(symbols):
    ...
    st.caption(
        "V9.5.4 uses the official NSE Indices NIFTY 500 constituent file first, then Upstox NSE_EQ mapping with instrument-search repair for valid equities. "
    ...
    st.caption("Confirmation layer for the focused daily watchlist. V9.5.4 does not seed or poll detailed 1-minute OHLC for all 500 stocks; it seeds strict daily candidates + strongest near-misses, then confirms them with live 1-minute structure.")
    with UPSTOX_WS_LOCK:
    ...
    if last_poll:
        st.caption(f"Latest Upstox I1 REST poll: {last_poll} server time • freshness limit: {RADAR_MAX_CANDLE_AGE_SECONDS:.0f}s")
...
def main():
    st.title('🎯 Pre-Breakout Hunter V9.5.4')
