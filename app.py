...
st.set_page_config(page_title='Pre-Breakout Hunter V9.5.6', page_icon='🎯', layout='wide')
...
def _start_radar_seed_for_watchlist(watchlist):
    """Attach the daily scan watchlist to the persistent live engine and seed 1m history.

    V9.5.4 fixes the V9.5.3 scan path where this helper was referenced but not
    defined. The watchlist is also copied into the persistent engine state so
    the background I1 poller immediately starts following these symbols.
    """
    clean=[]
    seen=set()
    ...
    with UPSTOX_WS_LOCK:
        ...
        running=bool(UPSTOX_WS_STATE.get("running"))
        existing=UPSTOX_WS_STATE.get("seed_thread")
        seed_running=bool(UPSTOX_WS_STATE.get("seed_running"))
        ...
        if seed_running and existing is not None and getattr(existing, "is_alive", lambda: False)():
            return True, f"Radar already seeding {len(focused)} focused stocks."
    ...
    t=threading.Thread(target=_seed_upstox_intraday_history, args=(focused,), name="upstox-radar-history-seed", daemon=True)
    with UPSTOX_WS_LOCK:
        UPSTOX_WS_STATE["seed_thread"] = t
    t.start()
    return True, f"Started focused radar history seed for {len(focused)} stocks."


def _poll_upstox_i1_ohlc_loop(mapped):
    """Stable live engine: 500-stock LTP + focused intraday 1-minute candles.

    V9.5.6 changes the focused radar source from Market Quote OHLC/I1 to the
    documented Intraday Candle endpoint. That endpoint returns today's 1-minute
    candles and is a better source for a rolling 5/10/20-minute confirmation.
    Only the focused radar watchlist is polled for detailed candles.
    """
    try:
        token = str(st.secrets.get("UPSTOX_ACCESS_TOKEN", "")).strip()
    except Exception:
    ...
    while True:
        ...
        # ---- Full-universe LTP: one request for up to 500 instruments. ----
        if now >= next_ltp:
            try:
                r = requests.get(
        ...
        # ---- Focused radar: live 1-minute OHLC from Market Quote V3. ----
        # This is intentionally a single request for the small focused watchlist.
        # It replaces the per-symbol intraday-history polling that could return
        # stale data or trigger unnecessary request bursts.
        if watchlist and now >= next_candle:
            try:
                focused_keys = [mapped.get(sym) for sym in watchlist if mapped.get(sym)]
                url = "https://api.upstox.com/v3/market-quote/ohlc"
                r = requests.get(
                    url, headers=headers,
                    params={"instrument_key": ",".join(str(x) for x in focused_keys), "interval": "I1"},
                    timeout=15,
                )
                if r.status_code == 429:
                    raise requests.HTTPError("HTTP 429 Too Many Requests", response=r)
                r.raise_for_status()
                payload = r.json()
                parsed_rows, latest_rows, parsed_count, shape = _extract_i1_response(payload, reverse)
                total_rows = 0
                total_candles = parsed_count
                first_error = None
                for sym, rows in parsed_rows.items():
                    if not rows:
                        continue
                    _update_candle_history({sym: rows[-120:]})
                    total_rows += 1
                    latest = rows[-1]
                    with UPSTOX_WS_LOCK:
                ...
                if total_rows == 0:
                    first_error = "V3 OHLC I1 returned no parseable 1-minute candles"
                with UPSTOX_WS_LOCK:
                    ...
                    UPSTOX_WS_STATE["ohlc_watchlist_size"] = len(watchlist)
                    UPSTOX_WS_STATE["ohlc_error"] = first_error or ""
                    UPSTOX_WS_STATE["ohlc_shape"] = shape
                candle_backoff = 0.0
                next_candle = time.time() + candle_interval
            except Exception as exc:
                errors.append(f"CANDLES: {type(exc).__name__}: {exc}")
                if getattr(getattr(exc, "response", None), "status_code", None) == 429:
                    candle_backoff = min(180.0, max(45.0, (candle_backoff or 45.0) * 2.0))
                next_candle = time.time() + (candle_backoff or 30.0)
...
if hasattr(st, "fragment"):
    ...
    def render_upstox_nifty500_panel():
        ...
        if ohlc_last_poll:
            st.caption(f"V3 I1 OHLC last poll: {ohlc_last_poll} • {ohlc_symbols}/{UPSTOX_WS_STATE.get("ohlc_watchlist_size", 0)} radar stocks • {ohlc_candles} candle rows parsed")
        if ohlc_error:
...
def _radar_data_health(candles):
    """Summarize freshness of the latest 1-minute candle for the focused radar set."""
    ages=[]
    symbols=0
    fresh=0
    for symbol, series in (candles or {}).items():
        ...
        if np.isfinite(age):
            ...
            if age <= RADAR_MAX_CANDLE_AGE_SECONDS:
                fresh += 1
    if not ages:
        return {"status":"NO DATA", "fresh":0, "symbols":0, "max_age":np.nan, "median_age":np.nan}
    max_age=max(ages)
    median_age=float(np.median(ages))
    status="LIVE" if fresh == symbols else ("DELAYED" if fresh > 0 else "STALE")
    return {"status":status, "fresh":fresh, "symbols":symbols, "max_age":max_age, "median_age":median_age}
...
def upstox_nifty500_stream_test(symbols):
    ...
    st.caption(
        "V9.5.6 uses the official NSE Indices NIFTY 500 constituent file first, then Upstox NSE_EQ mapping with instrument-search repair for valid equities. "
    ...
    st.caption("Confirmation layer for the focused daily watchlist. V9.5.6 does not seed or poll detailed 1-minute OHLC for all 500 stocks; it seeds strict daily candidates + strongest near-misses, then confirms them with live 1-minute structure.")
    with UPSTOX_WS_LOCK:
    ...
    health = _radar_data_health(radar_candles)
...
def main():
    st.title('🎯 Pre-Breakout Hunter V9.5.6')
