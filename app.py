...
def prepare_validation_features(d, nifty):
    """Precompute all rolling features once per stock for fast walk-forward validation."""
    d = d.sort_index()
    ...
    breakout_now = c >= pivot
    recent_breakout = breakout_now.rolling(3).max().fillna(False).astype(bool)
    last10_low = l.rolling(10).min()
    prev10_low = last10_low.shift(10)
    higher_low = last10_low >= prev10_low*0.985
    ...
    stock20 = c/c.shift(20)-1
    nifty20 = nn/nn.shift(20)-1
    rs = (stock20-nifty20)*100
    trend_ok = (c>sma50) & (sma50>sma150) & (sma150>sma200) & (sma200>sma200.shift(20)) & (c>ema20)
    base_ok = (r5 <= DEFAULTS['range5']*1.25) & (r10 <= DEFAULTS['range10']*1.20) & (r20 <= DEFAULTS['range20']*1.20)
    pre_breakout = (c < pivot) & (distance >= 0.20) & (distance <= 7.0)
    structural = trend_ok & base_ok & pre_breakout & (~recent_breakout) & higher_low & (~distribution)
    return {
        'close':c,'high':h,'low':l,'volume':v,'sma50':sma50,'sma150':sma150,'sma200':sma200,
        'ema20':ema20,'atr5':atr5,'atr20':atr20,'r5':r5,'r10':r10,'r20':r20,'vol5':vol5,'vol20':vol20,
        'pivot':pivot,'distance':distance,'higher_low':higher_low,'distribution':distribution,
        'rs':rs,'trend_ok':trend_ok,'base_ok':base_ok,'pre_breakout':pre_breakout,
        'recent_breakout':recent_breakout,'structural':structural
    }

def score_fast_at(f, i, cfg):
    vals = [f[k].iloc[i] for k in ['close','sma50','sma150','sma200','ema20','atr5','atr20','r5','r10','r20','vol5','vol20','pivot','distance','rs']]
    if not all(np.isfinite(x) for x in vals):
        return None
    c=f['close'].iloc[i]; r5=f['r5'].iloc[i]; r10=f['r10'].iloc[i]; r20=f['r20'].iloc[i]
    atr_ratio=f['atr5'].iloc[i]/f['atr20'].iloc[i] if f['atr20'].iloc[i] else np.inf
    vol_ratio=f['vol5'].iloc[i]/f['vol20'].iloc[i] if f['vol20'].iloc[i] else np.inf
    distance=f['distance'].iloc[i]; rs=f['rs'].iloc[i]
    score=0
    score += 10 if c>f['sma50'].iloc[i] else 0
    score += 8 if f['sma50'].iloc[i]>f['sma150'].iloc[i] else 0
    score += 8 if f['sma150'].iloc[i]>f['sma200'].iloc[i] else 0
    score += 5 if f['sma200'].iloc[i]>f['sma200'].iloc[i-20] else 0
    score += 4 if c>f['ema20'].iloc[i] else 0
    score += 5 if r5<=4.0 else (3 if r5<=cfg['range5'] else 0)
    score += 5 if r10<=7.0 else (3 if r10<=cfg['range10'] else 0)
    score += 5 if r20<=12.0 else (3 if r20<=cfg['range20'] else 0)
    score += 10 if r5<r10<r20 else (5 if r5<r10 or r10<r20 else 0)
    ...
    if 0.5<=distance<=2.5: score+=10
    elif cfg['pivot_min']<=distance<=cfg['pivot_max']: score+=7
    elif 0.2<=distance<=7.0: score+=4
    score += 5 if rs>=6 else (3 if rs>=3 else (1 if rs>=0 else 0))
    score += 5 if not bool(f['recent_breakout'].iloc[i]) else 0
    score=min(100,int(score))
    strict=bool(f['structural'].iloc[i]) and score>=cfg['min_score']
    return {'date':f['close'].index[i],'close':float(c),'pivot':float(f['pivot'].iloc[i]),'score':score,'distance':float(distance),'rs':float(rs),'strict':strict}
...
def historical_outcome(df, i, setup, breakout_window=10, holding_window=15):
    ...
    for j in range(i+1,end_break+1):
        if df['close'].iloc[j] > setup['pivot']:
            breakout_i=j; break
    if breakout_i is None or breakout_i+1>=len(df):
        return {'breakout':False,'days_to_breakout':np.nan,'entry':np.nan,'max_gain_pct':np.nan,'max_drawdown_pct':np.nan,'false_breakout':False,'breakout_volume_ratio':np.nan}
    entry_i=breakout_i+1
    entry=float(df['open'].iloc[entry_i])
    ...
    vol20=df['volume'].iloc[max(0,breakout_i-19):breakout_i+1].mean()
    br_vol=float(df['volume'].iloc[breakout_i]/vol20) if vol20 else np.nan
    return {'breakout':True,'days_to_breakout':breakout_i-i,'entry':entry,'max_gain_pct':max_gain,'max_drawdown_pct':max_dd,'false_breakout':false_breakout,'breakout_volume_ratio':br_vol}

def run_historical_validation(symbols, data, nifty, cfg, lookback_days=90, breakout_window=10, holding_window=15, progress_callback=None):
    """Fast walk-forward validation. Rolling indicators are computed once per stock."""
    rows=[]
    total=len(symbols)
    for si,sym in enumerate(symbols):
        try:
            ...
            f=prepare_validation_features(d,n)
            last_i=len(d)-breakout_window-2
            first_i=max(MIN_BARS-1,last_i-lookback_days+1)
            for i in range(first_i,last_i+1):
                setup=score_fast_at(f,i,cfg)
                if setup is None or not setup['strict']: continue
                outcome=historical_outcome(d,i,setup,breakout_window,holding_window)
                if outcome is None: continue
                rows.append({'Stock':sym,'Setup Date':setup['date'].strftime('%Y-%m-%d'),'Score':setup['score'],'To Pivot %':round(setup['distance'],2),'RS vs Nifty %':round(setup['rs'],2),**outcome})
        except Exception:
            pass
        if progress_callback: progress_callback((si+1)/total)
...
''')
    ...
    cfg = dict(DEFAULTS)
    cfg.update({'range5': max_range5, 'range10': max_range10, 'range20': max_range20,
                'atr_ratio': max_atr, 'vol_ratio': max_vol, 'pivot_min': pivot_min,
                'pivot_max': pivot_max, 'min_score': min_score})

    # Streamlit reruns the script whenever any widget is clicked. Keep the scan
    # and validation data in session_state so opening the expander or running the
    # backtest does not wipe the current scan.
    state_defaults = {
        'scan_done': False, 'scan_cfg': None, 'scan_symbols': None,
        'scan_data': None, 'scan_nifty': None, 'scan_all_out': None,
        'validation_result': None, 'validation_error': None,
    }
    ...
    if scan:
        ...
        st.session_state.scan_symbols = tuple(symbols)
        st.session_state.scan_data = data
        st.session_state.scan_nifty = nifty
        st.session_state.scan_all_out = all_out
    ...
    # Restore the last completed scan after every widget-triggered rerun.
    cfg = st.session_state.scan_cfg
    symbols = st.session_state.scan_symbols
    data = st.session_state.scan_data
    nifty = st.session_state.scan_nifty
    all_out = st.session_state.scan_all_out
    ...
    if strict_out.empty:
        ...
        st.dataframe(strict_out[['Stock','Score','Close','5D Range %','10D Range %','20D Range %','ATR5/ATR20','Vol5/Vol20','Pivot','To Pivot %','RS vs Nifty %','Status']], use_container_width=True, hide_index=True)
        st.download_button('⬇️ Download A-grade results CSV', strict_out.drop(columns=['_strict']).to_csv(index=False).encode('utf-8'), 'prebreakout_results.csv', 'text/csv')
    ...
    if near_out.empty:
        ...
        st.dataframe(near_out[['Stock','Score','Close','5D Range %','10D Range %','20D Range %','ATR5/ATR20','Vol5/Vol20','To Pivot %','RS vs Nifty %','Why it missed']], use_container_width=True, hide_index=True)
        st.download_button('⬇️ Download diagnostic CSV', near_out.drop(columns=['_strict']).to_csv(index=False).encode('utf-8'), 'prebreakout_diagnostic.csv', 'text/csv')

    st.markdown('### 📈 Historical Validation')
    st.caption("Fast walk-forward test: indicators are precomputed once per stock, and each historical setup is evaluated using only information available on that setup day. A breakout means the closing price first moved above that day's pivot within the selected window; entry is the next day open.")
    with st.expander('Run historical validation', expanded=False):
        lookback = st.slider('Historical setup days', 30, 120, 90, 10, key='bt_lookback')
        breakout_window = st.slider('Breakout window (days)', 5, 20, 10, 1, key='bt_breakout')
        holding_window = st.slider('Post-entry measurement window (days)', 5, 30, 15, 1, key='bt_holding')
        ...
        if run_bt:
            ...
            try:
                with st.spinner('Testing historical pre-breakout setups across the NIFTY 500...'):
                    result = run_historical_validation(tuple(symbols), data, nifty, cfg, lookback, breakout_window, holding_window, progress_callback=lambda p: progress_bar.progress(min(1.0, max(0.0, p))))
        ...
        if st.session_state.validation_error:
            st.error(f'Historical validation failed: {st.session_state.validation_error}')

        bt = st.session_state.validation_result
        if bt is not None:
            st.success(f'Historical validation completed — {len(bt)} setup event(s) tested.')
            if bt.empty:
                ...
                total = len(bt)
                br = int(bt['breakout'].sum())
                br_rate = br / total * 100
                ...
                bt_view = bt.copy()
                bt_view['Score Band'] = pd.cut(bt_view['Score'], bins=[-1,79,84,89,94,100], labels=['<80','80–84','85–89','90–94','95–100'])
                band = bt_view.groupby('Score Band', observed=False).agg(Setups=('Stock','size'), Breakout_Rate=('breakout','mean'), Median_Max_Gain=('max_gain_pct','median'), False_Breakout_Rate=('false_breakout','mean')).reset_index()
