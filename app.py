...
def _start_upstox_nifty500_websocket(symbols):
    ...
    V9.3.3 keeps the WebSocket on LTPC as an optional enhancement because the same
    ...
    "ohlc_thread": None, "ohlc_running": False, "ohlc_error": "", "ohlc_last_poll": None, "ohlc_symbols": 0, "ohlc_candles": 0, "rest_live": False, "mapped": mapped,
...
if hasattr(st, "fragment"):
    ...
    def render_upstox_nifty500_panel():
        ...
        if ohlc_last_poll:
            st.caption(f"V3 I1 OHLC last poll: {ohlc_last_poll} • {ohlc_symbols}/500 symbols • {ohlc_candles} candle rows parsed • one 500-key REST request per cycle")
        if ohlc_error:
...
def upstox_nifty500_stream_test(symbols):
    ...
    st.caption(
        "V9.3.3 uses the official NSE Indices NIFTY 500 constituent file first, then Upstox NSE_EQ mapping with instrument-search repair for valid equities. "
    ...
    st.caption("V9.3.3 uses Upstox V3 I1 REST as the authoritative 1-minute engine. WebSocket LTPC is optional; a WebSocket 403 no longer stops the intraday engine.")

def main():
    st.title('🎯 Pre-Breakout Hunter V9.3.3')
