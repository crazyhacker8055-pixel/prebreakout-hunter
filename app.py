...
def _seed_upstox_intraday_history(mapped):
    ...
    with ThreadPoolExecutor(max_workers=12,thread_name_prefix="upstox-seed") as pool:
...
def _start_upstox_nifty500_websocket(symbols):
    ...
    thread.start(); ohlc_thread.start(); seed_thread.start()
...
if hasattr(st, "fragment"):
    ...
    def render_upstox_nifty500_panel():
        ...
        if connected and rest_live:
            st.success("🟢 LIVE — WebSocket LTP + Upstox V3 I1 REST")
        elif connected:
            st.success("🟢 LIVE — Upstox WebSocket V3 connected to NIFTY 500")
        elif rest_live:
            st.success("🟢 LIVE — Upstox V3 I1 REST market engine (WebSocket fallback)")
        elif running:
        ...
        if error:
            st.warning(f"WebSocket status: {error}")
        if ohlc_last_poll:
        ...
        if seed_running:
            st.info(f"⏳ Seeding today's 1-minute history: {seed_ok}/{seed_total} stocks loaded. Radar will become meaningful as history builds.")
        elif seed_done:
...
def main():
    st.title('🎯 Pre-Breakout Hunter V9.4.1')
