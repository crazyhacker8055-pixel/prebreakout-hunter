...
st.set_page_config(page_title='Pre-Breakout Hunter V8.5', page_icon='🎯', layout='wide')
...
def run_validation_batch(symbols, nifty, cfg, batch_no, batch_size, setup_days, bw, td, progress=None):
    ...
    end=datetime.now(); start=end-timedelta(days=750)
    start_s=start.strftime('%Y-%m-%d'); end_s=(end+timedelta(days=1)).strftime('%Y-%m-%d')
...
def main():
    st.title('🎯 Pre-Breakout Hunter V8.5')
...
V8 deliberately rewards *readiness* rather than simply giving points for a good-looking trend. Results are **watchlist candidates, not automatic buy signals**.''')
    ...
    st.markdown('### 📈 V8.5 Long Walk-Forward Trade Validation')
    st.caption('V8.5 runs historical validation in small resumable batches. This avoids one long Streamlit connection and limits Yahoo Finance rate pressure. Each completed batch is kept in the current session.')
    with st.expander('Run V8.5 historical trade validation', expanded=bool(st.session_state.validation_status)):
        setup_days=st.slider('Historical setup days per batch',25,100,50,25,key='val_setup_days')
        ...
        def do_one(batch_no):
            ...
            hist_nifty_frames=download_one_batch(('^NSEI',), (datetime.now()-timedelta(days=750)).strftime('%Y-%m-%d'), (datetime.now()+timedelta(days=1)).strftime('%Y-%m-%d'))
