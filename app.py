...
st.set_page_config(page_title="Pre-Breakout Hunter V22", page_icon="🎯", layout="wide")
...
def live_regrade(structure_df, frames, live_map, mapping, min_score=84, near_score=72, now=None, near_limit=10):
    ...
    near=((out["LTP"]<out["Pivot"]) & (out["Live To Pivot %"]>=0) & (out["Live To Pivot %"]<=7.0) & ((out["Score"]>=near_score) | (out["Live Breakout Pressure"]>=35)))
...
def main():
    ...
    def candidate_cards(df,title,limit=10):
        ...
        for _,r in df.head(limit).iterrows():
            ...
            with st.container(border=True):
                ...
                y1,y2=st.columns(2); y1.metric("Live Breakout Pressure",f"{int(r.get('Live Breakout Pressure',0))}/100"); y2.metric("Live Position",f"{float(r.get('Live Position %',0)):.0f}%")
    ...
    def show_table(df,title):
        ...
        cols=["Stock","Stage","Score","Live Breakout Pressure","LTP","Planned Entry","Stop Loss","Target 1 (2R)","Target 2 (3R)","Target 3 (4R)","Risk %","Risk Status","Live To Pivot %","Live Range %","7D Range %","20D Range %","Live Signal Reason","Occurrences","Validated success %"]
...
""")
    st.caption("Pre-Breakout Hunter V22 • LIVE-FIRST full-universe engine • Upstox Analytics Token • no Yahoo • no order/trading API")
