...
st.set_page_config(page_title='Pre-Breakout Hunter V8.8', page_icon='🎯', layout='wide')
...
def main():
    st.title('🎯 Pre-Breakout Hunter V8.8')
...
V8 deliberately rewards *readiness* rather than simply giving points for a good-looking trend. Results are **watchlist candidates, not automatic buy signals**.''')
    ...
    if st.session_state.scan_ran and not st.session_state.market_data.empty:
        ...
        t1,t2,t3,t4,t5=st.tabs(['🔥 Pre-Breakout + Sector','🚀 Top Gainers','🔊 Volume Gainers','🗺️ Sector Strength','🔻 Top Losers'])
        with t1:
