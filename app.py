...
st.set_page_config(page_title="Pre-Breakout Hunter V25", page_icon="🎯", layout="wide")

APP_VERSION = "V25.0 FINAL — BREAKOUT-CATCH ENGINE"
...
def fetch_history_batch(jobs, start, end, token_fingerprint):
    """Fast, cache-first history fetch.

    Upstox permits up to 50 standard API requests/sec. We stay just below that
    limit and use 24 workers. Each symbol is cached independently, so later
    scans on the same completed session reuse the history instead of downloading
    495 daily series again.
    """
    frames, errors = {}, {}
    last_req = [0.0]
...
NEWS_POSITIVE = [
    "order win", "work order", "order received", "contract", "award", "project",
    "approval", "approved", "launch", "partnership", "acquisition", "merger",
    "stake", "investment", "capex", "expansion", "new facility", "capacity",
    "revenue", "profit", "ebitda", "earnings", "results", "dividend", "buyback",
    "government", "regulatory", "licence", "license", "clearance", "strategic",
    "deal", "business win", "client", "mandate", "tender", "ipo", "funding"
]
NEWS_NEGATIVE = [
    "fraud", "default", "insolvency", "penalty", "probe", "investigation", "raid",
    "resign", "resignation", "downgrade", "loss", "fire", "pledge", "warning",
    "recall", "lawsuit", "ban", "suspend", "suspension", "delay", "go slow"
]
NEWS_STRONG = [
    "order win", "work order", "contract", "approval", "acquisition", "merger",
    "government", "results", "revenue", "profit", "capex", "launch", "stake",
    "regulatory", "clearance", "strategic deal", "business win"
]

def _news_text(item):
    return (str(item.get("heading", "")) + " " + str(item.get("summary", ""))).strip()

def score_news_item(item, now=None):
    now = now or now_ist()
    text = _news_text(item).lower()
    if not text:
        return 0, ""
    try:
        ts = pd.to_datetime(item.get("published_time"), unit="ms", utc=True).tz_convert(IST)
        age_h = max(0.0, (now - ts.to_pydatetime()).total_seconds()/3600.0)
    except Exception:
        age_h = 168.0
    pos = sum(1 for k in NEWS_POSITIVE if k in text)
    neg = sum(1 for k in NEWS_NEGATIVE if k in text)
    strong = sum(1 for k in NEWS_STRONG if k in text)
    recency = 35 if age_h <= 4 else 25 if age_h <= 12 else 18 if age_h <= 24 else 10 if age_h <= 72 else 4
    score = recency + min(35, pos*8) + min(20, strong*7) - min(45, neg*18)
    return int(np.clip(score, 0, 100)), text

@st.cache_data(ttl=NEWS_CACHE_TTL, show_spinner=False)
def fetch_news_batch(keys_tuple, token_fingerprint):
    """Fetch Upstox News API for <=30 instrument keys. Analytics Token supports this read-only API."""
    keys=list(keys_tuple)
    if not keys:
        return {}
    try:
        r=requests.get(NEWS_URL, headers=headers(), params={
            "category":"instrument_keys", "instrument_keys":",".join(keys),
...
@st.cache_data(ttl=NEWS_CACHE_TTL, show_spinner=False)
def fetch_all_news(mapping, token_fingerprint):
    """Fetch the last 7 days of news for the complete mapped universe in 30-key chunks."""
    keys=[str(x) for x in mapping.dropna(subset=["instrument_key"]).instrument_key.tolist() if str(x).strip()]
    out={}
    for i in range(0,len(keys),30):
        chunk=tuple(keys[i:i+30])
        data=fetch_news_batch(chunk, token_fingerprint)
        out.update(data)
    return out

def build_news_map(news_payload, mapping, now=None):
    now=now or now_ist()
    key_to_sym=dict(zip(mapping.instrument_key,mapping.Symbol))
    rows=[]
    for key,items in (news_payload or {}).items():
        sym=key_to_sym.get(key)
        if not sym or not isinstance(items,list):
            continue
        scored=[]
        for item in items:
            if not isinstance(item,dict): continue
            sc,text=score_news_item(item,now)
            try:
                ts=pd.to_datetime(item.get("published_time"),unit="ms",utc=True).tz_convert(IST)
                age_h=max(0.0,(now-ts.to_pydatetime()).total_seconds()/3600.0)
            except Exception:
                ts=pd.NaT; age_h=np.inf
            scored.append((sc,age_h,item,text,ts))
        scored.sort(key=lambda z:(z[0],-z[1]),reverse=True)
        if not scored:
            rows.append({"Stock":sym,"News Score":0,"News Count":0,"News Fresh":"NO","News Catalyst":"NONE","Latest News":"","News Published":pd.NaT})
            continue
        top=scored[0]
        sc,age_h,item,text,ts=top
        fresh=age_h<=24
        catalyst="STRONG" if sc>=60 else "POSITIVE" if sc>=35 else "NONE"
        rows.append({
            "Stock":sym,"News Score":int(sc),"News Count":len(scored),
            "News Fresh":"YES" if fresh else "NO","News Catalyst":catalyst,
            "Latest News":str(item.get("heading","")).strip(),"News Published":ts
        })
    return pd.DataFrame(rows) if rows else pd.DataFrame(columns=["Stock","News Score","News Count","News Fresh","News Catalyst","Latest News","News Published"])
...
def live_regrade(structure_df, frames, live_map, mapping, news_df=None, min_score=84, near_score=72, now=None, near_limit=10):
    """LIVE-FIRST classification designed not to miss explosive resistance tests.

    The structural score remains strict for READY. A separate event-driven path can
    create a NEAR-MISS when current price action is exceptional even if the older
    long-term trend gate is imperfect. This is specifically intended to catch names
    like PROTEAN: session high through resistance + abnormal volume/range + strong
    intraday position, while current LTP may temporarily sit back below resistance.
    """
    ...
    if news_df is not None and not news_df.empty:
        nf=news_df.copy(); nf["Stock"]=nf["Stock"].astype(str).str.strip().str.upper()
        out=out.merge(nf,on="Stock",how="left")
    else:
        out["News Score"]=0; out["News Count"]=0; out["News Fresh"]="NO"; out["News Catalyst"]="NONE"; out["Latest News"]=""

    out["News Score"]=out["News Score"].fillna(0).astype(float)
    out["News Count"]=out["News Count"].fillna(0).astype(int)
    ...
    out["Day Change %"]=np.nan; out["Live Range / 20D Avg"]=np.nan; out["Live Volume / 20D Avg"]=np.nan; out["Live To 52W High %"]=np.nan
    for i,r in out.iterrows():
        ...
        out.at[i,"Day Change %"]=(float(r["LTP"])/prev_close-1)*100 if prev_close else np.nan
    ...
    pressure += np.where(out["Live Volume / 20D Avg"]>=2.0,8,np.where(out["Live Volume / 20D Avg"]>=1.5,5,np.where(out["Live Volume / 20D Avg"]>=1.0,2,0)))
    pressure += np.where(out["News Score"]>=60,8,np.where(out["News Score"]>=35,4,0))
    pressure += np.where(out["Live Fresh"]==False,-30,0)
    out["Live Breakout Pressure"]=pressure.clip(0,100).round().astype(int)
    out["Live Score"]=(0.65*base+0.35*out["Live Breakout Pressure"]).round().clip(0,100).astype(int)

    # Independent event detector. It does NOT lower the READY threshold.
    # It catches a stock whose intraday high has already tested/broken resistance,
    # while current LTP has pulled back below it, provided the move is exceptional.
    resistance_test=(out["Live High"]>=out["Pivot"]*0.997) | ((out["Live To Pivot %"]>=0)&(out["Live To Pivot %"]<=4.0))
    abnormal_volume=out["Live Volume / 20D Avg"]>=1.50
    abnormal_range=out["Live Range / 20D Avg"]>=1.20
    strong_position=out["Live Position %"]>=65
    fresh=out["Live Fresh"]==True
    catalyst=(out["News Score"]>=35)&(out["News Fresh"]=="YES")
    event_quality=fresh & resistance_test & ((abnormal_volume & strong_position) | (abnormal_range & strong_position) | (out["Live High"]>=out["Pivot"]) )
    event_quality=event_quality & ((out["Live Breakout Pressure"]>=48) | catalyst)
    # Don't label a very old, already distant move as a near-miss.
    event_quality=event_quality & (out["Live To Pivot %"]>=-3.0)

    strict_structure=out["Score"]>=min_score
    breakout=(out["LTP"]>=out["Pivot"]) & strict_structure & fresh & (out["Live Breakout Pressure"]>=55)
    ready=(out["LTP"]<out["Pivot"])&(out["Live To Pivot %"]>=0)&(out["Live To Pivot %"]<=3.5)&strict_structure&(out["Live Breakout Pressure"]>=45)&fresh
    normal_near=(out["LTP"]<out["Pivot"])&(out["Live To Pivot %"]>=0)&(out["Live To Pivot %"]<=5.0)&(out["Score"]>=near_score)
    # Event-driven near miss takes precedence over WATCH but not over a true READY/BREAKOUT.
    # STRICT MODE: event_quality NEVER changes READY/NEAR-MISS. It is a separate radar tab.
    near=normal_near
    out["Event Catch"]=event_quality

    reasons=[]; stages=[]
    for _,r in out.iterrows():
        rr=[]; d=float(r.get("Live To Pivot %",np.nan)) if pd.notna(r.get("Live To Pivot %",np.nan)) else np.nan
        if pd.notna(d): rr.append("price above resistance" if d<=0 else "within 1% of resistance" if d<=1 else "within 2% of resistance" if d<=2 else "near resistance" if d<=5 else "far from resistance")
        if float(r.get("Live High",0) or 0)>=float(r.get("Pivot",0) or 0): rr.append("session high broke/tested pivot")
        elif str(r.get("Live Pivot Pressure","NO"))=="YES": rr.append("session high testing pivot")
        if float(r.get("Live Volume / 20D Avg",0) or 0)>=2: rr.append("volume surge")
        elif float(r.get("Live Volume / 20D Avg",0) or 0)>=1.5: rr.append("volume expansion")
        if float(r.get("Live Range / 20D Avg",0) or 0)>=1.2: rr.append("range expansion")
        if str(r.get("News Catalyst","NONE"))!="NONE": rr.append("fresh catalyst news")
        if float(r.get("Score",0) or 0)<min_score: rr.append("structure below READY threshold")
        reasons.append(" • ".join(rr[:4]) if rr else "insufficient live pressure")
        if bool(event_quality.loc[r.name]):
            stages.append("NEAR-MISS NOW • BREAKOUT TESTED")
        elif bool(breakout.loc[r.name]): stages.append("BREAKOUT NOW")
        elif bool(ready.loc[r.name]): stages.append("READY NOW")
        elif bool(normal_near.loc[r.name]): stages.append("NEAR-MISS NOW")
        else: stages.append("WATCH")
    # Rebuild explicit precedence because event_quality can coexist with breakout/ready.
    stages=[]
    for i,r in out.iterrows():
        if bool(breakout.loc[i]): stages.append("BREAKOUT NOW")
        elif bool(ready.loc[i]): stages.append("READY NOW")
        elif bool(near.loc[i]): stages.append("NEAR-MISS NOW")
        else: stages.append("WATCH")
    out["Live Signal Reason"]=reasons
    out["Stage"]=stages
    out["Live Status"]=["⚡ LIVE BREAKOUT NOW" if x=="BREAKOUT NOW" else "🟢 READY NOW" if x=="READY NOW" else "🟡 NEAR-MISS NOW" if x=="NEAR-MISS NOW" else "WATCH" for x in stages]
    out["Event Status"]=np.where(event_quality,"🚨 EVENT CATCH — BREAKOUT TESTED","—")
    ...
    out=out.sort_values(["_stage_order","Live Priority Score","Live Breakout Pressure","News Score","Live To Pivot %"],ascending=[True,False,False,False,True]).drop(columns=["_stage_order"])
    return out
...
def add_live_priority(df):
    ...
    news=out.get("News Score",pd.Series(0,index=out.index)).fillna(0).astype(float)
    out["Historical Adjusted Success %"]=adj
    out["Evidence Grade"]=[evidence_grade(n) for n in occ]
    out["Live Priority Score"]=(
        0.32*pressure + 0.22*structure + 0.14*proximity +
        0.09*position + 0.08*compression + 0.05*evidence + 0.10*news
...
def main():
    ...
    if scan and st.session_state.v21_job is None:
        ...
        st.session_state.v21_job=dict(
            ...
            jobs=jobs, idx=0, batch_size=HISTORY_BATCH_SIZE, frames={}, errors={}, cfg=cfg,
    ...
    if job is not None:
        ...
        if idx < total:
            ...
            st.caption("Fast mode: 100 stocks/batch, 24 workers, 48 requests/sec (below Upstox standard 50 req/sec). Each symbol history is cached independently, so repeat scans reuse completed daily data.")
            try:
            ...
            try:
                ...
                status.write("3/5 — Reading FRESH Upstox 1D + 1-minute OHLC for ALL mapped stocks…")
                ...
                status.write("4/5 — Checking LIVE resistance tests, volume/range expansion and fresh NEWS catalysts…")
                try:
                    news_payload=fetch_all_news(mapping,token_fp())
                    news_df=build_news_map(news_payload,mapping,now_ist())
                except Exception:
                    news_payload={}; news_df=pd.DataFrame()
                live_df=live_regrade(pool_df,frames,live_map,mapping,news_df=news_df,min_score=cfg["min_score"],near_score=cfg["near_score"],now=now_ist())
                if live_df.empty:
                    ready_df=pd.DataFrame(columns=result_cols); near_df=pd.DataFrame(columns=result_cols); triggered_df=pd.DataFrame(columns=result_cols); event_df=pd.DataFrame(columns=result_cols)
                else:
                    ...
                    event_df=live_df[(live_df["Event Catch"]==True) & (live_df["Live Fresh"]==True)].copy()
                    event_df=event_df.sort_values(["Live Breakout Pressure","Live Priority Score","News Score","Live To Pivot %"],ascending=[False,False,False,True])
                    # Event radar is deliberately separate. It never promotes a stock into
                    # READY/NEAR-MISS and therefore does not loosen the original scanner.
                status.write("5/5 — Validating only the strongest LIVE candidates historically…")
                names_df=pd.concat([x[["Stock"]] for x in (triggered_df,ready_df,near_df,event_df) if not x.empty],ignore_index=True) if any(not x.empty for x in (triggered_df,ready_df,near_df,event_df)) else pd.DataFrame(columns=["Stock"])
                ...
                if not bt.empty and "Stock" in bt.columns:
                    ...
                    if not event_df.empty: event_df=event_df.merge(bt,on="Stock",how="left")
                ...
                event_df=add_live_priority(event_df)
                for _df_name in ("ready_df","near_df","triggered_df","event_df"):
                    _df=locals()[_df_name]
                    if not _df.empty:
                ...
                st.session_state.v21=dict(ready=ready_df,near=near_df,triggered=triggered_df,event=event_df,frames=frames,errors=errors,live_errors=live_errors,nifty=nifty,mapping=mapping,start=start,end=end,asof=asof,cfg=cfg,run_time=now_ist(),live_map=live_map,news_df=news_df)
                st.session_state.v21_job=None
    ...
    if state is not None and state.get("run_time") is not None:
        current_trade_date=last_completed_weekday()
        if state.get("asof") != current_trade_date:
            st.warning(f"⚠️ Previous scan is from {state.get('asof')}. It is NOT being presented as today's live result. Press RUN FULL SCANNER for {current_trade_date}.")
    if state is None:
    ...
    event=state.get("event",pd.DataFrame())
    news_df_state=state.get("news_df",pd.DataFrame())
    ...
    news_count=int((news_df_state["News Score"]>=35).sum()) if not news_df_state.empty and "News Score" in news_df_state.columns else 0
    st.caption(f"Data: {len(state['frames'])}/{mapped_count} mapped histories usable • LIVE scan time: {state['run_time'].strftime('%Y-%m-%d %H:%M:%S %Z')} • Live OHLC: Upstox V3 current session • News: {news_count} mapped stocks with catalyst-level news")
    st.info("STRICT MODE: READY and NEAR-MISS use the original structure rules. 🚨 LIVE EVENT RADAR is separate and does NOT promote stocks into the strict lists. This keeps the scanner strict while still exposing exceptional PROTEAN-type live moves.")
    ...
    def candidate_cards(df,title,limit=10):
        ...
        for _,r in df.head(limit).iterrows():
            ...
            with st.container(border=True):
                ...
                if str(r.get("Latest News","")).strip():
                    st.caption(f"📰 News ({r.get('News Catalyst','NONE')}, score {int(r.get('News Score',0))}): {r.get('Latest News','')}")
                st.caption(f"Day change {float(r.get('Day Change %',0)):.2f}% • Live volume / 20D {float(r.get('Live Volume / 20D Avg',0)):.2f}x • Live range / 20D {float(r.get('Live Range / 20D Avg',0)):.2f}x")
    ...
    candidate_cards(ready,"🎯 READY NOW — strict setup",5)
    candidate_cards(near,"🟡 NEAR-MISS NOW — strict setup",5)
    candidate_cards(triggered,"⚡ LIVE BREAKOUT NOW — strict setup",5)

    tab_strict, tab_event, tab_news = st.tabs(["🎯 STRICT SCANNER", "🚨 LIVE EVENT RADAR", "📰 NEWS RADAR"])
    # Results above remain the default strict dashboard. The event radar is intentionally separate.
    with tab_event:
        st.subheader("🚨 LIVE EVENT RADAR — separate from the strict scanner")
        st.caption("This tab NEVER promotes a stock into READY or NEAR-MISS. It highlights exceptional live resistance/volume/range events so a PROTEAN-type move is visible without weakening the strategy.")
        if event.empty:
            st.success("No exceptional live event detected in the current scan.")
        else:
            for _,r in event.head(10).iterrows():
                with st.container(border=True):
                    st.markdown(f"### {r['Stock']} • {r.get('Event Status','EVENT')}")
                    a,b=st.columns(2); a.metric("LTP",f"₹{float(r['LTP']):,.2f}"); b.metric("Event Pressure",f"{int(r.get('Live Breakout Pressure',0))}/100")
                    c,d=st.columns(2); c.metric("To Pivot",f"{float(r.get('Live To Pivot %',np.nan)):.2f}%"); d.metric("Live Position",f"{float(r.get('Live Position %',np.nan)):.0f}%")
                    st.caption(f"Structure score: {int(r.get('Score',0))}/100 • Live range/20D: {float(r.get('Live Range / 20D Avg',0)):.2f}x • Live volume/20D: {float(r.get('Live Volume / 20D Avg',0)):.2f}x")
                    st.caption(f"Why caught: {r.get('Live Signal Reason','—')}")
                    if str(r.get('Latest News','')).strip(): st.caption(f"📰 {r.get('News Catalyst','NONE')} • {r.get('Latest News','')}")
            st.dataframe(event.head(20)[[c for c in ['Stock','Live Breakout Pressure','Score','LTP','Pivot','Live To Pivot %','Live Range / 20D Avg','Live Volume / 20D Avg','Live Position %','News Score','News Catalyst','Latest News'] if c in event.columns]],width='stretch',hide_index=True)
    with tab_news:
        st.subheader("📰 NEWS RADAR — catalyst context only")
        st.caption("News never creates a strict candidate by itself. It is shown separately so catalyst information cannot loosen the setup rules.")
        if news_df_state.empty:
            st.info("No mapped news data returned for this scan.")
        else:
            st.dataframe(news_df_state.sort_values(['News Score','News Count'],ascending=[False,False]).head(50),width='stretch',hide_index=True)
...
- **LIVE PRIORITY:** 32% live breakout pressure, 22% structure, 14% resistance proximity, 9% live position, 8% live compression, 5% historical evidence, 10% news catalyst.
- **Event-catch layer:** a stock may enter NEAR-MISS if the session high tests/breaks resistance and there is abnormal volume/range + strong intraday position, even when the long-term trend score is below READY. Fresh catalyst news can reinforce this event path.
- **News:** Upstox News API is checked for the mapped universe (last 7 days); recent catalyst headlines are scored but never used alone to create a candidate.
...
""")
    st.caption("Pre-Breakout Hunter V26 • STRICT scanner + separate LIVE EVENT RADAR • Upstox Analytics Token • no Yahoo • no order/trading API")
