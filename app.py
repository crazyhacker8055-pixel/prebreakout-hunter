...
st.set_page_config(page_title='Pre-Breakout Hunter V8.3', page_icon='🎯', layout='wide')
...
def run_validation_batch(symbols, nifty, cfg, batch_no, batch_size, setup_days, bw, td, progress=None):
    ...
    failed=[s for s in batch if f'{s}.NS' not in frames]
    return bt,failed,batch

def main():
    st.title('🎯 Pre-Breakout Hunter V8.3')
...
V8 deliberately rewards *readiness* rather than simply giving points for a good-looking trend. Results are **watchlist candidates, not automatic buy signals**.''')
    ...
    if scan_btn:
        ...
        with st.spinner('Downloading current daily data in small batches...'):
            ...
            frames=download_batches_resumable(tuple(symbols),period_days=520,batch_size=100,progress=dlp)
            dlp.empty()
    ...
    st.markdown('### 📈 V8.3 Long Walk-Forward Trade Validation')
    st.caption('V8.3 runs historical validation in small resumable batches. This avoids one long Streamlit connection and limits Yahoo Finance rate pressure. Each completed batch is kept in the current session.')
    with st.expander('Run V8.3 historical trade validation', expanded=bool(st.session_state.validation_status)):
        ...
        if not bt.empty:
            ...
            if not trades.empty:
                ...
                st.download_button('⬇️ Download V8.3 validation CSV',bt.to_csv(index=False).encode(),'v8_3_validation.csv','text/csv')
        else:
