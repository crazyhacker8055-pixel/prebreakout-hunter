...
def main():
    ...
    st.info("LIVE PRIORITY ranks what is closest to a real breakout right now. Structure score is only the base; live pressure and resistance proximity carry the most weight. Historical success is sample-adjusted so tiny samples cannot dominate.")
    ...
    def candidate_cards(df,title,limit=10):
        ...
        for _,r in df.head(limit).iterrows():
            ...
            with st.container(border=True):
                ...
                st.caption(f"Backtest events: {occ} • Raw success: {success} • Adjusted success: {adj_txt} • {r.get('Evidence Grade','—')}")
...
""")
    st.caption("Pre-Breakout Hunter V23 • LIVE-PRIORITY full-universe engine • Upstox Analytics Token • no Yahoo • no order/trading API")
