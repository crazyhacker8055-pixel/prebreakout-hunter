...
st.set_page_config(page_title='Pre-Breakout Hunter V8.4', page_icon='🎯', layout='wide')
...
def download_one_batch(batch, start_date, end_date):
    """Download one Yahoo batch with consistent .NS ticker normalization and gentle retries."""
    import time
    batch = tuple(batch)
    ...
    for attempt, wait in enumerate((0, 5, 15, 30)):
        ...
        try:
            d = yf.download(list(yahoo_batch), start=start_date, end=end_date, interval='1d',
                            auto_adjust=False, group_by='column', threads=False, progress=False, timeout=30)
            frames = {}
...
def run_validation_batch(symbols, nifty, cfg, batch_no, batch_size, setup_days, bw, td, progress=None):
    batch=validation_batch_symbols(symbols,batch_no,batch_size)
    if not batch:
        return pd.DataFrame(), [], batch
    end=datetime.now(); start=end-timedelta(days=1100)
    start_s=start.strftime('%Y-%m-%d'); end_s=(end+timedelta(days=1)).strftime('%Y-%m-%d')
    ...
    bt=backtest_fast(frames,nifty,cfg,setup_days,bw,td,progress=None)
    failed=[s for s in batch if f'{s}.NS' not in frames and s not in frames]
    return bt,failed,batch

def main():
    st.title('🎯 Pre-Breakout Hunter V8.4')
...
V8 deliberately rewards *readiness* rather than simply giving points for a good-looking trend. Results are **watchlist candidates, not automatic buy signals**.''')
    ...
    if scan_btn:
        ...
        with st.spinner('Downloading current daily data in small batches...'):
            ...
            frames=download_batches_resumable(tuple(symbols),period_days=520,batch_size=25,progress=dlp)
            dlp.empty()
    ...
    st.markdown('### 📈 V8.4 Long Walk-Forward Trade Validation')
    st.caption('V8.4 runs historical validation in small resumable batches. This avoids one long Streamlit connection and limits Yahoo Finance rate pressure. Each completed batch is kept in the current session.')
    with st.expander('Run V8.4 historical trade validation', expanded=bool(st.session_state.validation_status)):
        setup_days=st.slider('Historical setup days per batch',50,200,100,50,key='val_setup_days')
        ...
        def do_one(batch_no):
            ...
            bt,failed,batch=run_validation_batch(symbols,hist_nifty,cfg,batch_no,batch_size,setup_days,bw,td,progress)
        ...
        if run_all:
            # Run all batches sequentially, but each network call is only 25 stocks.
        ...
        if not bt.empty:
            ...
            if not trades.empty:
                ...
                st.dataframe(bt.sort_values('Setup Date',ascending=False).head(200),width='stretch',hide_index=True)
                st.download_button('⬇️ Download V8.3 validation CSV',bt.to_csv(index=False).encode(),'v8_4_validation.csv','text/csv')
        else:
            st.info('No historical setup events collected yet. Run the current batch; if a batch has no usable Yahoo data, it will be reported instead of crashing the app.')
