...
st.set_page_config(page_title="Pre-Breakout Hunter V16", page_icon="🎯", layout="wide")
...
def main():
    ...
    a.metric("READY",len(ready)); b.metric("NEAR-MISS",len(near)); c.metric("TRIGGERED",len(triggered)); d.metric("BACKTESTED",min(30,backtested_count))
    mapped_count=len(state['mapping'].dropna(subset=['instrument_key']))
    st.caption(f"Data: {len(state['frames'])}/{mapped_count} mapped histories usable • Scan: {state['run_time'].strftime('%Y-%m-%d %H:%M:%S %Z')} • Setup date: {state['asof']} • Live OHLC: Upstox V3.")
    ...
    def candidate_cards(df,title,limit=10):
        ...
        for _,r in df.head(limit).iterrows():
            ...
            with st.container(border=True):
                ...
                x2.metric("Historical success",success)
                x3,x4=st.columns(2)
                x3.metric("To Pivot",f"{float(r['Live To Pivot %']):.2f}%" if pd.notna(r.get('Live To Pivot %',np.nan)) else "—")
                x4.metric("Live Range",f"{float(r['Live Range %']):.2f}%" if pd.notna(r.get('Live Range %',np.nan)) else "—")
    ...
    if not chart_df.empty:
        ...
        st.caption("Blue candles are completed sessions. The final highlighted candle is today's evolving Upstox live OHLC. The horizontal line is the prior 20-session resistance/pivot.")
        try:
        ...
        for sym in chart_df.Stock.head(5).tolist():
            ...
            if fig is not None:
                fig.add_trace(go.Candlestick(
                    ...
                    name="Completed candles", increasing_line_color="#22c55e", decreasing_line_color="#ef4444"))
                ...
                if live_ts.tzinfo is not None:
                    live_ts=live_ts.tz_convert(IST).tz_localize(None)
                live_open=float(r["Live Open"]); live_high=float(r["Live High"]); live_low=float(r["Live Low"]); live_close=float(r["LTP"])
                fig.add_trace(go.Candlestick(
                    x=[live_ts], open=[live_open], high=[live_high], low=[live_low], close=[live_close],
                    name="LIVE today", increasing_line_color="#00e676", decreasing_line_color="#ff9800"))
                pivot=float(r["Pivot"])
                fig.add_hline(y=pivot,line_dash="dash",line_width=2,annotation_text=f"Pivot ₹{pivot:,.2f}",annotation_position="top left")
                fig.add_hline(y=live_close,line_dash="dot",line_width=1,annotation_text=f"LTP ₹{live_close:,.2f}",annotation_position="bottom left")
                lows=list(hist["low"].astype(float))+[live_low,pivot,live_close]
                highs=list(hist["high"].astype(float))+[live_high,pivot,live_close]
                lo=min(lows); hi=max(highs); pad=max((hi-lo)*0.08, max(abs(hi),1)*0.003)
                fig.update_layout(height=360,margin=dict(l=10,r=10,t=35,b=10),xaxis_rangeslider_visible=False,showlegend=True,
                                  title=f"{sym} • Score {int(r.Score)} • {r.get('Live Status','')} • 7D range {float(r['7D Range %']):.2f}%")
                fig.update_yaxes(range=[lo-pad,hi+pad],fixedrange=False,tickformat=",.0f")
                st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False})
                st.caption(f"Today's live OHLC: O ₹{live_open:,.2f} • H ₹{live_high:,.2f} • L ₹{live_low:,.2f} • LTP ₹{live_close:,.2f} • Live range {float(r['Live Range %']):.2f}% • Live/20D avg {float(r['Live Range / 20D Avg']):.2f}x • To pivot {float(r['Live To Pivot %']):.2f}%")
            else:
...
""")
    ...
    with st.expander("🩺 Data health",expanded=False):
        ...
        if usable_count == mapped_count:
            st.success(f"All {mapped_count} mapped stocks returned sufficient completed daily history.")
        else:
            st.warning(f"{usable_count} of {mapped_count} mapped stocks returned sufficient completed history. {mapped_count-usable_count} mapped stocks were excluded from scoring because usable history was insufficient.")
