...
st.set_page_config(page_title='Pre-Breakout Hunter V9.2', page_icon='🎯', layout='wide')
...
UPSTOX_WS_STATE = {
    ...
    "updates": {}, "mapped": {}, "unmapped": [], "universe_size": 0,
}
...
def build_upstox_nifty500_mapping(symbols):
    """Map our NIFTY 500 symbols to current Upstox NSE_EQ instrument keys."""
...
def _start_upstox_nifty500_websocket(symbols):
    ...
    try:
        mapped, unmapped, _ = build_upstox_nifty500_mapping(symbols)
    ...
    with UPSTOX_WS_LOCK:
        UPSTOX_WS_STATE.update({
            ...
            "unmapped": unmapped, "universe_size": len(symbols),
        })
    ...
    def worker():
        ...
        try:
            ...
            # LTPC is sufficient for this mapping/streaming stage and supports thousands of keys.
            streamer = upstox_client.MarketDataStreamerV3(api_client, keys, "ltpc")
            with UPSTOX_WS_LOCK:
            ...
            def on_message(message):
                updates = _extract_live_updates(message, reverse)
                with UPSTOX_WS_LOCK:
    ...
    thread = threading.Thread(target=worker, name="upstox-v3-nifty500", daemon=True)
    with UPSTOX_WS_LOCK:
...
if hasattr(st, "fragment"):
    ...
    def render_upstox_nifty500_panel():
        ...
        if connected:
            ...
            st.info("🟡 Connecting full NIFTY 500 stream…")
        else:
...
def upstox_nifty500_stream_test(symbols):
    ...
    st.caption(
        "V9.2 maps the current NIFTY 500 list to Upstox NSE_EQ instrument keys and streams the full universe using LTPC. "
    ...
    st.caption("This stage validates universe mapping and full-universe streaming before we attach the live data to the pre-breakout scoring engine.")

def main():
    st.title('🎯 Pre-Breakout Hunter V9.2')
