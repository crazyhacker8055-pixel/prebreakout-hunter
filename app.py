...
st.set_page_config(page_title='Pre-Breakout Hunter V9.5.7', page_icon='🎯', layout='wide')
...
def _get_upstox_persistent_state():
    # V9.5.3: Streamlit reruns recreate module globals. Keep the live engine
    # state in a cache_resource so a daily scan/rerun does not make the
    # already-running background market engine appear STOPPED.
    return {
...
@st.cache_resource(show_spinner=False)
def _get_upstox_persistent_lock():
    return threading.Lock()

UPSTOX_WS_STATE = _get_upstox_persistent_state()
UPSTOX_WS_LOCK = _get_upstox_persistent_lock()
...
def _extract_i1_response(payload, reverse):
    """Parse Upstox V3 Intraday Candle safely across dict/list response shapes."""
    rows, latest = {}, {}
    parsed = 0
    shape = []
...
def _start_radar_seed_for_watchlist(watchlist):
    """Attach the focused radar watchlist to the live engine.

    V9.5.7 does not launch a separate seed request burst. The documented
    Upstox V3 Intraday Candle endpoint already returns the current trading
    day's 1-minute history, so the live focused poller populates the history
    directly. This removes duplicate requests and eliminates the old seed/
    poll race that contributed to 429/server instability.
    """
    clean=[]
    seen=set()
    for x in (watchlist or []):
        sym=str(x).upper().strip()
        if sym and sym not in seen:
            seen.add(sym); clean.append(sym)
    clean=clean[:30]
    if not clean:
        return False, "No focused radar candidates are available."
    with UPSTOX_WS_LOCK:
        mapped=dict(UPSTOX_WS_STATE.get("mapped", {}))
        focused={sym:mapped[sym] for sym in clean if sym in mapped}
        UPSTOX_WS_STATE["radar_watchlist"] = list(focused.keys())
        UPSTOX_WS_STATE["radar_watchlist_source"] = "daily strict candidates + strongest near-misses"
        UPSTOX_WS_STATE["seed_running"] = False
        UPSTOX_WS_STATE["seed_done"] = True
        UPSTOX_WS_STATE["seed_ok"] = 0
        UPSTOX_WS_STATE["seed_total"] = len(focused)
        UPSTOX_WS_STATE["seed_error"] = "No separate seed required — V3 Intraday Candle polling returns current-day 1-minute history."
        if not focused:
            return False, "Radar candidates were found, but none are currently mapped to Upstox NSE_EQ instruments."
    return True, f"Focused radar watchlist ready: {len(focused)} stocks. Live polling will load current-day 1-minute history."

def _fetch_intraday_history_one(symbol, ikey, token):
    """Fetch today's 1-minute candles for one focused radar symbol.

    Upstox V3 documents this endpoint as the current-trading-day intraday
    candle source. It returns rows as [timestamp, open, high, low, close,
    volume, open-interest]. We keep only the most recent 120 rows locally.
    """
    url = "https://api.upstox.com/v3/historical-candle/intraday/{}/minutes/1".format(
        requests.utils.quote(str(ikey), safe="")
    )
    ...
    if response.status_code == 429:
        err = requests.HTTPError("HTTP 429 Too Many Requests", response=response)
        raise err
    ...
    for c in raw:
        if isinstance(c, (list, tuple)) and len(c) >= 6:
            try:
                ...
                continue
    rows.sort(key=lambda x: str(x.get("1m Timestamp", "")))
    return rows[-120:]


def _poll_upstox_i1_ohlc_loop(mapped):
    """Stable live engine: 500-stock LTP + focused current-day 1-minute candles.

    V9.5.7 removes the fragile Market Quote I1 parser from the radar path.
    Instead, the focused watchlist uses the documented V3 Intraday Candle
    endpoint. This is deliberately limited to the small radar list, not all
    500 stocks. Four workers are used only for the focused list so a 16-stock
    radar remains responsive without creating the burst that caused the earlier
    server/429 problems.
    """
    try:
        token = str(st.secrets.get("UPSTOX_ACCESS_TOKEN", "")).strip()
    ...
    if not token or not keys:
        with UPSTOX_WS_LOCK:
            UPSTOX_WS_STATE["ohlc_error"] = "Missing token or mapped instruments for Upstox V3 market data."
    ...
    while True:
        with UPSTOX_WS_LOCK:
            if not UPSTOX_WS_STATE.get("ohlc_running"):
                break
            watchlist = [x for x in UPSTOX_WS_STATE.get("radar_watchlist", []) if x in mapped]

        now = time.time()
        errors = []

        # Full-universe LTP: one request for up to 500 instruments every 30s.
        if now >= next_ltp:
            try:
                r = requests.get(
                    "https://api.upstox.com/v3/market-quote/ltp",
                    headers=headers,
                    params={"instrument_key": ",".join(str(x) for x in keys)},
                    timeout=15,
                )
                if r.status_code == 429:
                    raise requests.HTTPError("HTTP 429 Too Many Requests", response=r)
                r.raise_for_status()
                payload = r.json()
                data = payload.get("data", {}) if isinstance(payload, dict) else {}
                if isinstance(data, dict):
                    with UPSTOX_WS_LOCK:
                        ...
                        for api_key, obj in data.items():
                            if not isinstance(obj, dict):
                                continue
                            ikey = str(obj.get("instrument_token") or api_key).replace(":", "|")
                            sym = reverse.get(ikey) or reverse.get(str(api_key).replace(":", "|"))
                            if not sym:
                                continue
                            last_price = obj.get("last_price")
                            prev_close = obj.get("cp")
                            try:
                                change_pct = ((float(last_price) / float(prev_close)) - 1.0) * 100.0 if prev_close not in (None, 0, "0") else None
                            except Exception:
                                change_pct = None
                            row = updates.get(sym, {"Symbol": sym})
                            row.update({
                                "Symbol": sym, "Instrument Key": ikey,
                                "LTP": last_price, "Prev Close": prev_close,
                                "Change %": change_pct,
                                "Received": datetime.now().strftime("%H:%M:%S"),
                            })
                        ...
                        UPSTOX_WS_STATE["rest_live"] = bool(updates)
                        UPSTOX_WS_STATE["ohlc_last_ltp_poll"] = datetime.now().strftime("%H:%M:%S")
                        UPSTOX_WS_STATE["ohlc_ltp_symbols"] = len(data)
                ltp_backoff = 0.0
                next_ltp = time.time() + ltp_interval
            except Exception as exc:
                errors.append(f"LTP: {type(exc).__name__}: {exc}")
                if getattr(getattr(exc, "response", None), "status_code", None) == 429:
                    ltp_backoff = min(120.0, max(30.0, (ltp_backoff or 30.0) * 2.0))
                next_ltp = time.time() + (ltp_backoff or 30.0)

        # Focused radar: current trading-day 1-minute candles.
        # One request per focused symbol, max 4 in parallel. This is intentionally
        # NOT a 500-symbol intraday poll.
        if watchlist and now >= next_candle:
            from concurrent.futures import ThreadPoolExecutor, as_completed
            focused = [(sym, mapped[sym]) for sym in watchlist if mapped.get(sym)]
            successful = 0
            parsed_count = 0
            first_error = None
            rate_limited = False
            ...
            def fetch(item):
                sym, ikey = item
                return sym, _fetch_intraday_history_one(sym, ikey, token)

            try:
                with ThreadPoolExecutor(max_workers=min(4, max(1, len(focused)))) as executor:
                    futures = [executor.submit(fetch, item) for item in focused]
                    for future in as_completed(futures):
                        try:
                            sym, rows = future.result()
                            if rows:
                                _update_candle_history({sym: rows})
                                parsed_count += len(rows)
                                successful += 1
                                latest = rows[-1]
                                with UPSTOX_WS_LOCK:
                                    base = UPSTOX_WS_STATE.setdefault("updates", {}).get(sym, {"Symbol": sym})
                                    base.update(latest)
                                    base["1m Received"] = datetime.now().strftime("%H:%M:%S")
                                    UPSTOX_WS_STATE["updates"][sym] = base
                            elif first_error is None:
                                first_error = f"{sym}: Upstox returned no 1-minute candles"
                        except Exception as exc:
                            if getattr(getattr(exc, "response", None), "status_code", None) == 429:
                                rate_limited = True
                ...
                with UPSTOX_WS_LOCK:
                    ...
                    UPSTOX_WS_STATE["ohlc_symbols"] = successful
                    UPSTOX_WS_STATE["ohlc_candles"] = parsed_count
                    UPSTOX_WS_STATE["ohlc_watchlist_size"] = len(focused)
                    UPSTOX_WS_STATE["ohlc_error"] = first_error or ""
                    UPSTOX_WS_STATE["ohlc_source"] = "V3 Intraday Candle Data (current trading day)"

                if rate_limited:
                    candle_backoff = min(180.0, max(60.0, (candle_backoff or 60.0) * 2.0))
                    errors.append("CANDLES: Upstox HTTP 429 Too Many Requests — backing off")
                    next_candle = time.time() + candle_backoff
                else:
                    candle_backoff = 0.0
                    next_candle = time.time() + candle_interval
            except Exception as exc:
                errors.append(f"CANDLES: {type(exc).__name__}: {exc}")
                next_candle = time.time() + (candle_backoff or candle_interval)

        if errors:
            with UPSTOX_WS_LOCK:
                # Keep a useful diagnostic without hiding successful LTP operation.
                UPSTOX_WS_STATE["ohlc_error"] = " | ".join(errors)[:700]
                UPSTOX_WS_STATE["rest_live"] = bool(UPSTOX_WS_STATE.get("updates"))
...
def _start_upstox_nifty500_websocket(symbols):
    ...
    try:
        existing_session_watchlist = [str(x).upper().strip() for x in st.session_state.get("radar_watchlist", []) if str(x).strip()]
    except Exception:
        existing_session_watchlist = []
    existing_session_watchlist = existing_session_watchlist[:30]
    with UPSTOX_WS_LOCK:
    ...
    "ohlc_thread": None, "ohlc_running": False, "ohlc_error": "", "ohlc_last_poll": None, "ohlc_symbols": 0, "ohlc_candles": 0, "rest_live": False,
            "seed_thread": None, "seed_running": False, "seed_done": False, "seed_ok": 0, "seed_total": 0, "seed_error": "", "radar_watchlist": [x for x in existing_session_watchlist if x in mapped], "radar_watchlist_source": "existing daily scan watchlist", "mapped": mapped,
    ...
    return True, f"started with {len(mapped)} mapped instruments + V3 Intraday Candle polling"
...
if hasattr(st, "fragment"):
    ...
    def render_upstox_nifty500_panel():
        ...
        if connected and rest_live:
            st.success("🟢 LIVE — Upstox V3 Intraday Candle REST (stable mode)")
        elif connected:
            st.success("🟢 LIVE — Upstox Upstox V3 REST market engine")
        elif rest_live:
            st.success("🟢 LIVE — Upstox V3 Intraday Candle REST market engine (stable REST mode)")
        elif running:
        ...
        if opened_at:
            st.caption(f"Connected at {opened_at} server time. Feed mode: LTPC (read-only) • live LTP stream. 1-minute OHLCV is fetched from Upstox V3 Intraday Candle REST.")
        if error:
            st.caption(f"WebSocket optional status: {error}")
        if ohlc_last_poll:
            st.caption(f"V3 intraday 1-minute last poll: {ohlc_last_poll} • {ohlc_symbols}/{UPSTOX_WS_STATE.get("ohlc_watchlist_size", 0)} radar stocks • {ohlc_candles} candle rows parsed")
        if ohlc_error:
            st.warning(ohlc_error)
        if seed_running:
            st.info(f"⏳ Loading focused radar history: {seed_ok}/{seed_total} stocks loaded from the current-day intraday endpoint.")
        elif seed_done and seed_total:
            st.success(f"✅ Focused radar watchlist ready: {seed_total} stocks. Current-day 1-minute history is loaded by the live intraday poller.")
        with UPSTOX_WS_LOCK:
            wl=list(UPSTOX_WS_STATE.get("radar_watchlist", []))
        if wl:
            st.caption(f"🎯 Radar watchlist: {len(wl)} stocks • {', '.join(wl[:12])}{' …' if len(wl)>12 else ''}")
        if seed_error:
            st.caption(f"Radar history note: {seed_error[:300]}")
        if unmapped:
        ...
        if updates:
            ...
            display_df = df[preferred].copy() if preferred else df.copy()
            if '1m Timestamp' in display_df.columns:
                display_df['1m Timestamp'] = display_df['1m Timestamp'].astype(str)
            ...
            if ch:
                ...
                st.caption("Upstox V3 supplies current NIFTY 500 LTP/volume snapshots; detailed 1-minute OHLCV is polled only for the focused radar watchlist and kept in memory for intraday confirmation.")
                if not cdf.empty:
                    ...
                    display_cdf = cdf[display_cols].head(30).copy() if display_cols else cdf.head(30).copy()
                    if '1m Timestamp' in display_cdf.columns:
                        display_cdf['1m Timestamp'] = display_cdf['1m Timestamp'].astype(str)
                    st.dataframe(display_cdf, use_container_width=True, hide_index=True, height=420)
                else:
...
def _parse_upstox_timestamp(value):
    """Return a timezone-aware UTC Timestamp for an Upstox candle timestamp."""
    try:
        if value is None or (isinstance(value, float) and np.isnan(value)):
            return None
        # Upstox timestamps are normally ISO-8601 strings. Handle epoch seconds/ms too.
...
def _candle_age_seconds(value):
    ts = _parse_upstox_timestamp(value)
    if ts is None:
        return np.nan
...
RADAR_MAX_CANDLE_AGE_SECONDS = 150.0


def _radar_data_health(candles, expected_symbols=None):
    """Summarize freshness against the full focused radar watchlist.

    A stock with no candle is counted as missing, rather than silently
    shrinking the denominator. This prevents a partial API response from
    being displayed as LIVE.
    """
    ages=[]
    available=0
    fresh=0
    expected=[str(x).upper().strip() for x in (expected_symbols or []) if str(x).strip()]
    symbols=len(expected) if expected else len(candles or {})
    iterator=((sym, (candles or {}).get(sym, [])) for sym in expected) if expected else (candles or {}).items()
    for symbol, series in iterator:
        if isinstance(series, dict):
            series=[series]
        ...
        if not last:
            continue
        age=_candle_age_seconds(last.get("1m Timestamp"))
        if np.isfinite(age):
            available += 1
            ages.append(float(age))
            if age <= RADAR_MAX_CANDLE_AGE_SECONDS:
                fresh += 1
    if not ages:
        return {"status":"NO DATA", "fresh":0, "available":0, "symbols":symbols, "max_age":np.nan, "median_age":np.nan}
    max_age=max(ages)
    median_age=float(np.median(ages))
    status="LIVE" if (fresh == symbols and available == symbols) else ("DELAYED" if fresh > 0 else "STALE")
    return {"status":status, "fresh":fresh, "available":available, "symbols":symbols, "max_age":max_age, "median_age":median_age}
...
def build_intraday_radar(candles, daily_candidates=None, max_rows=25):
    ...
    for symbol, series in (candles or {}).items():
        ...
        try:
            ...
            if len(d) < 20:
                continue
            latest_ts = d["1m Timestamp"].iloc[-1] if "1m Timestamp" in d.columns else None
            age_sec = _candle_age_seconds(latest_ts)
            # Never generate a live setup from an old candle. During market hours
            # the current 1-minute bar can be up to ~60s old; 150s gives a small
            # API/network grace window without allowing stale signals.
            if not np.isfinite(age_sec) or age_sec > RADAR_MAX_CANDLE_AGE_SECONDS:
                continue
            close = d["1m Close"]
            high = d["1m High"]
            low = d["1m Low"]
            ...
            rows.append({
                ...
                "Data Age (sec)": round(float(age_sec), 0),
...
def upstox_nifty500_stream_test(symbols):
    ...
    st.caption(
        "V9.5.7 uses the official NSE Indices NIFTY 500 constituent file first, then Upstox NSE_EQ mapping with instrument-search repair for valid equities. "
    ...
    if c1.button("▶️ START NIFTY 500 LIVE", type="primary", width="stretch"):
        ...
        if ok: st.toast(msg)
            ...
            if msg == 'already running': st.info('🟢 NIFTY 500 live engine is already running — no action needed.')
    ...
    c3.caption("Stable design: LTP is polled for NIFTY 500; current-day 1-minute history is fetched only for the focused radar watchlist.")
    with UPSTOX_WS_LOCK:
        persistent_watchlist = list(UPSTOX_WS_STATE.get("radar_watchlist", []))
    if persistent_watchlist:
        if st.button("🎯 SEED / REFRESH RADAR HISTORY", width="stretch"):
            ok_seed,msg_seed=_start_radar_seed_for_watchlist(persistent_watchlist)
    ...
    st.caption("Confirmation layer for the focused daily watchlist. V9.5.7 does not request detailed 1-minute history for all 500 stocks; it fetches current-day 1-minute candles only for strict daily candidates + strongest near-misses, then confirms them with live 1-minute structure.")
    with UPSTOX_WS_LOCK:
        radar_candles = {k: list(v) for k, v in UPSTOX_WS_STATE.get("candles", {}).items()}
        rest_live_now = bool(UPSTOX_WS_STATE.get("rest_live"))
        last_poll = UPSTOX_WS_STATE.get("ohlc_last_poll")
    with UPSTOX_WS_LOCK:
        expected_watchlist = list(UPSTOX_WS_STATE.get("radar_watchlist", []))
    health = _radar_data_health(radar_candles, expected_watchlist)
    h1,h2,h3,h4 = st.columns(4)
    h1.metric("Radar Data", health["status"])
    h2.metric("Fresh Candles", f'{health["fresh"]}/{health["symbols"]}')
    h3.metric("Median Age", f'{health["median_age"]:.0f}s' if np.isfinite(health["median_age"]) else "—")
    h4.metric("Max Age", f'{health["max_age"]:.0f}s' if np.isfinite(health["max_age"]) else "—")
    if health["status"] == "LIVE":
        st.success("🟢 DATA FRESH — Radar is allowed to generate live confirmation setups.")
    elif health["status"] == "DELAYED":
        st.warning("🟡 DATA DELAYED — only fresh symbols are eligible; delayed symbols are excluded.")
    elif health["status"] == "STALE":
        st.error("🔴 DATA STALE — no intraday confirmation setup will be generated until fresh 1-minute data arrives.")
    else:
        st.info("⚪ WAITING FOR 1-MINUTE DATA — Radar is not evaluating entries yet.")
    if last_poll:
        st.caption(f"Latest Upstox intraday 1-minute poll: {last_poll} server time • freshness limit: {RADAR_MAX_CANDLE_AGE_SECONDS:.0f}s")
    ...
    if radar.empty:
        ...
        st.caption("Radar logic: progressive 5m/10m/20m contraction + low ATR + volume dry-up + higher lows + price pressing the prior 20-minute pivot + no recent 1-minute breakout + fresh-data gate (≤150s).")

def main():
    st.title('🎯 Pre-Breakout Hunter V9.5.7')
...
V8 deliberately rewards *readiness* rather than simply giving points for a good-looking trend. Results are **watchlist candidates, not automatic buy signals**.''')
    ...
    if scan_btn:
        ...
        # V9.5.4: after the daily scan, create and attach the focused intraday radar universe.
