...
def main():
    st.title('🎯 Pre-Breakout Hunter V8')
...
V8 deliberately rewards *readiness* rather than simply giving points for a good-looking trend. Results are **watchlist candidates, not automatic buy signals**.''')
    ...
    st.markdown('### 📈 V8 Long Walk-Forward Trade Validation')
    ...
    with st.expander('Run V8 historical trade validation'):
        ...
        if st.button('🧪 RUN V8 TRADE VALIDATION',width='stretch'):
            st.info('V8 validation downloads history in independently cached batches. If Yahoo rate-limits a batch, rerunning will reuse completed batches instead of starting over.')
            dlp=st.progress(0, text='Downloading historical data…')
            hist_frames=download_batches_resumable(tuple(symbols),period_days=1100,batch_size=100,progress=dlp)
            dlp.empty()
