...
st.set_page_config(page_title='Pre-Breakout Hunter V6', page_icon='🎯', layout='wide')
...
def download_market_data(symbols):
    ...
    # ~3 years of calendar history, enough for long walk-forward validation.
...
def render_validation(bt):
    ...
    st.download_button('⬇️ Download V6 validation CSV',bt.to_csv(index=False).encode(),'prebreakout_v6_validation.csv','text/csv')

def main():
    st.title('🎯 Pre-Breakout Hunter V6')
...
**V6:** strong trend → controlled base → progressive contraction → higher lows → repeated resistance pressure → improving relative strength → quiet volume → breakout still ahead.
...
**Breakout-readiness scoring** now rewards resistance tests, closing position inside the base, cleaner candles, relative-strength acceleration and healthier volume behavior. Historical testing uses only setup-day information.''')
    ...
    if scan:
        ...
        with st.spinner('Downloading ~3 years of daily market data and checking V6 setups...'):
    ...
    if strict.empty:
        ...
        st.download_button('⬇️ Download A-grade results CSV',strict.drop(columns=['_strict']).to_csv(index=False).encode(),'prebreakout_v6_results.csv','text/csv')
    ...
    if near.empty: st.info('No near-miss stocks were available.')
        ...
        st.download_button('⬇️ Download diagnostic CSV',near.drop(columns=['_strict']).to_csv(index=False).encode(),'prebreakout_v6_diagnostic.csv','text/csv')

    st.markdown('### 📈 V6 Long Walk-Forward Trade Validation')
    ...
    with st.expander('Run V6 historical trade validation',expanded=False):
        ...
        run=st.button('🧪 RUN V6 TRADE VALIDATION',use_container_width=True)
        if run:
            st.session_state.validation_result=None; st.session_state.validation_error=None; p=st.progress(0)
            try:
                with st.spinner('Testing V6 historical trades across the NIFTY 500...'):
