...
st.set_page_config(page_title='Pre-Breakout Hunter V7', page_icon='🎯', layout='wide')
    ...
    for t in tables:
        for c in t.columns:
            if 'symbol' in str(c).lower():
                syms = t[c].astype(str).str.strip().tolist()
...
def download_batches(symbols, period_days=520, batch_size=80):
    end = datetime.now()
    ...
    for i in range(0, len(tickers), batch_size):
        ...
        d = yf.download(batch, start=start.strftime('%Y-%m-%d'),
...
def backtest(frames,nifty,cfg,setup_days=250,breakout_window=10,trade_days=30):
    events=[]
    for sym,df in frames.items():
        if len(df)<MIN_BARS+breakout_window+trade_days: continue
        dates=df.index
        # Use evenly spaced recent setup dates to control cloud workload.
        usable=dates[MIN_BARS:-breakout_window-trade_days]
        if len(usable)>setup_days:
            idx=np.linspace(0,len(usable)-1,setup_days,dtype=int); usable=usable[idx]
        for dt in usable:
            try:
                r=evaluate(df,nifty,cfg,date=dt)
                if not r: continue
                after=df.loc[dt:].iloc[1:breakout_window+trade_days+1]
                if len(after)<breakout_window+1: continue
                pivot=r['Pivot']; br=None
                for j,(ix,row) in enumerate(after.iloc[:breakout_window].iterrows(),start=1):
                    if row.close>pivot:
                        br=(j,ix); break
                ev={'Stock':sym.replace('.NS',''),'Setup Date':dt.strftime('%Y-%m-%d'),'Score':r['Score'],'To Pivot %':r['To Pivot %']}
                if br is None:
                    ...
                    j,bdate=br; future=df.loc[bdate:].iloc[1:trade_days+2]
                    if future.empty: continue
                    entry=float(future.iloc[0].open)
                    # Exit rule: close below 220 SMA or -15% from entry; next open after trigger.
                    work=df.loc[bdate:].copy(); work['sma220']=work.close.rolling(220).mean()
                    exit_price=None; exit_date=None
                    for k in range(0,min(trade_days,len(work)-1)):
                        row=work.iloc[k]
                        trigger=(np.isfinite(row.sma220) and row.close<row.sma220) or row.close<=entry*0.85
                        if trigger:
                            exit_price=float(work.iloc[k+1].open); exit_date=work.index[k+1]; break
                    if exit_price is None:
                        exit_price=float(work.iloc[min(trade_days,len(work)-1)].close); exit_date=work.index[min(trade_days,len(work)-1)]
                    trade_ret=(exit_price/entry-1)*100
                    path=work.iloc[:min(trade_days+1,len(work))]
...
def main():
    st.title('🎯 Pre-Breakout Hunter V7')
    ...
    with st.sidebar:
        ...
        scan_btn=st.button('🚀 SCAN NIFTY 500',type='primary',use_container_width=True)
    st.markdown('''### Exact V7 setup being hunted
...
V7 deliberately rewards *readiness* rather than simply giving points for a good-looking trend. Results are **watchlist candidates, not automatic buy signals**.''')
    if not scan_btn:
    ...
    with st.spinner('Downloading current daily data in small batches...'):
        frames=download_batches(tuple(symbols),period_days=520,batch_size=80)
    nifty_df=yf.download('^NSEI',start=(datetime.now()-timedelta(days=520)).strftime('%Y-%m-%d'),end=(datetime.now()+timedelta(days=1)).strftime('%Y-%m-%d'),auto_adjust=False,progress=False)
    nifty=ohlcv_from_download(nifty_df,'^NSEI')['close']
    out,diag=scan(symbols,frames,nifty,cfg)
    if out.empty: st.warning('No strict V7 setups found today. This is intentional.');
    else:
        st.success(f'Found **{len(out)}** strict V7 pre-breakout candidate(s).')
        st.dataframe(out,use_container_width=True,hide_index=True)
        st.download_button('⬇️ Download A-grade CSV',out.to_csv(index=False).encode(),'v7_prebreakout_results.csv','text/csv')
    ...
    if diag.empty: st.info('No strong near-misses.')
        ...
        st.download_button('⬇️ Download diagnostic CSV',diag.to_csv(index=False).encode(),'v7_near_miss.csv','text/csv')
    st.markdown('### 📈 V7 Long Walk-Forward Trade Validation')
    ...
    with st.expander('Run V7 historical trade validation'):
        setup_days=st.slider('Historical setup days',100,500,250,50)
        bw=st.slider('Breakout window (days)',5,20,10)
        td=st.slider('Maximum observation / trade days',15,40,30)
        if st.button('🧪 RUN V7 TRADE VALIDATION',use_container_width=True):
            with st.spinner('Downloading up to 3 years of daily data in small batches... First run can take several minutes.'): 
                hist_frames=download_batches(tuple(symbols),period_days=1100,batch_size=60)
                hist_nifty_df=yf.download('^NSEI',start=(datetime.now()-timedelta(days=1100)).strftime('%Y-%m-%d'),end=(datetime.now()+timedelta(days=1)).strftime('%Y-%m-%d'),auto_adjust=False,progress=False)
                hist_nifty=ohlcv_from_download(hist_nifty_df,'^NSEI')['close']
            with st.spinner('Testing historical V7 setups using only setup-day information...'):
                bt=backtest(hist_frames,hist_nifty,cfg,setup_days,bw,td)
            if bt.empty: st.warning('No historical V7 setups were found in this sample.')
                ...
                if not trades.empty:
                    ...
                    st.dataframe(bt.sort_values('Setup Date',ascending=False).head(100),use_container_width=True,hide_index=True)
                    st.download_button('⬇️ Download V7 validation CSV',bt.to_csv(index=False).encode(),'v7_validation.csv','text/csv')
