...
st.set_page_config(page_title='Pre-Breakout Hunter V9.5.5', page_icon='🎯', layout='wide')
...
def _poll_upstox_i1_ohlc_loop(mapped):
    ...
    V9.5.5 changes the focused radar source from Market Quote OHLC/I1 to the
    ...
    while True:
        ...
        # ---- Focused radar: today's 1-minute candles, one request per symbol. ----
        if watchlist and now >= next_candle:
            try:
                    ...
                    try:
                        ...
                        if rows:
                            ...
                            if first_error is None:
                                first_error = f"{sym}: intraday endpoint returned no candles"
                    except Exception as exc:
                        if first_error is None:
                            first_error = f"{sym}: {type(exc).__name__}: {exc}"
                        if getattr(getattr(exc, "response", None), "status_code", None) == 429:
                            raise
                ...
                if getattr(getattr(exc, "response", None), "status_code", None) == 429:
                    candle_backoff = min(120.0, max(30.0, (candle_backoff or 30.0) * 2.0))
...
def upstox_nifty500_stream_test(symbols):
    ...
    st.caption(
        "V9.5.5 uses the official NSE Indices NIFTY 500 constituent file first, then Upstox NSE_EQ mapping with instrument-search repair for valid equities. "
    ...
    st.caption("Confirmation layer for the focused daily watchlist. V9.5.5 does not seed or poll detailed 1-minute OHLC for all 500 stocks; it seeds strict daily candidates + strongest near-misses, then confirms them with live 1-minute structure.")
    with UPSTOX_WS_LOCK:
...
def main():
    st.title('🎯 Pre-Breakout Hunter V9.5.5')
