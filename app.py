...
st.set_page_config(page_title="Pre-Breakout Hunter V27", page_icon="🎯", layout="wide")

APP_VERSION = "V27.0 FINAL — STRICT + LIVE EVENT RADAR + FAST REFRESH"
...
def live_regrade(structure_df, frames, live_map, mapping, min_score=84, near_score=72, now=None, near_limit=10):
    ...
    if ld.empty: return structure_df.copy(), pd.DataFrame()
    ...
    # ---------------- SEPARATE EVENT RADAR ----------------
    # This is deliberately independent of the strict structure score.
    # It requires a real resistance test plus live participation, so quiet
    # touches do not flood the event list.
    fresh=out["Live Fresh"]==True
    close_near=(out["Live To Pivot %"]>=-3.0)&(out["Live To Pivot %"]<=3.5)
    ...
    strong_participation=(range_ok|volume_ok)
    minute_test=(out["Minute High"]>=out["Pivot"]*0.997)|(out["Minute Close"]>=out["Pivot"]*0.997)
    breakout_now=fresh&(out["LTP"]>=out["Pivot"])&resistance_test&(out["Live Position %"]>=70)&(volume_ok| (range_ok & minute_test))
    tested=fresh&close_near&resistance_test&position_ok&strong_participation
    reclaim=fresh&(out["LTP"]>=out["Pivot"]*0.995)&resistance_test&(out["Live Position %"]>=65)&minute_test&volume_ok
    event_quality=breakout_now|reclaim|tested
    ev=out[event_quality].copy()
    if not ev.empty:
        ev["Event Status"]=np.select([breakout_now.loc[ev.index],reclaim.loc[ev.index],tested.loc[ev.index]],
                                       ["⚡ BREAKOUT NOW","🔄 BREAKOUT RECLAIMING","🚨 BREAKOUT TESTED"],default="🚨 BREAKOUT TESTED")
        # Quality score is for event ranking only; it never affects strict score.
        ev["Event Quality Score"]=(
            np.clip(ev["Live Breakout Pressure"],0,100)*0.35+
            np.clip(ev["Live Position %"],0,100)*0.20+
            np.clip(ev["Live Range / 20D Avg"].fillna(0)*50,0,100)*0.15+
            np.clip(ev["Live Volume / 20D Avg"].fillna(0)*40,0,100)*0.20+
            np.where(ev["Live High"]>=ev["Pivot"],100,70)*0.10
        ).round().clip(0,100).astype(int)
        ev["Event Reason"]=np.where(ev["LTP"]>=ev["Pivot"],"price is at/above resistance",
                             np.where(ev["Live High"]>=ev["Pivot"],"session high crossed resistance then price pulled back","resistance test in progress"))
        ev["Event Reason"]=ev["Event Reason"]+np.where(ev["Live Volume / 20D Avg"]>=1.25," • volume participation","" )+np.where(ev["Live Range / 20D Avg"]>=0.80," • range expansion","" )
        ev=ev.sort_values(["Event Quality Score","Live Breakout Pressure","Live Position %"],ascending=[False,False,False])
    return strict,ev
...
def perform_live_refresh(state, mapping, cfg):
    ...
    live_day,day_errors=live_ohlc(mapping.instrument_key.astype(str).tolist(),interval="1d")
...
def render_event_radar(event):
    ...
    st.caption("This tab NEVER promotes a stock into READY/NEAR-MISS. It only detects a meaningful live resistance test/breakout attempt from fresh Upstox data.")
    if event.empty:
...
def main():
    ...
    if job is not None:
        ...
        if idx<total:
            ...
            if len(frames)<490:
                st.error(f"Only {len(frames)}/{total} mapped stocks returned sufficient history. Scan blocked."); st.session_state.v27_job=None; return
            ...
            try:
                ...
                live_day,day_errors=live_ohlc(mapped.instrument_key.astype(str).tolist(),"1d"); live_min,minute_errors=live_ohlc(mapped.instrument_key.astype(str).tolist(),"I1")
                live_map=merge_live_maps(live_day,live_min)
...
**LIVE EVENT RADAR:** independent. It requires fresh data plus a genuine resistance test and meaningful intraday participation (range and/or volume), with position and 1-minute confirmation. It can detect a stock that tests resistance and pulls back without changing the strict scanner.
