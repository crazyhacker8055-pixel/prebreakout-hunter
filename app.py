...
def live_regrade(structure_df, frames, live_map, mapping, min_score=84, near_score=72, now=None, near_limit=10):
    ...
    if not ev.empty:
        ...
        for _,r in ev.iterrows():
            ...
            rr=[]
            status=str(r.get("Event Status",""))
            if status=="⚡ BREAKOUT NOW": rr.append("resistance crossed within 2%")
            elif status=="🟢 BREAKOUT CONFIRMING": rr.append("breakout 2–3% above resistance")
            elif status=="🔄 BREAKOUT RECLAIMING": rr.append("reclaiming resistance")
            elif status=="🚨 BREAKOUT TESTED": rr.append("session high crossed resistance; price pulled back")
            elif status=="🔵 EXTENDED BREAKOUT": rr.append("already >3% above resistance — extended")
            elif status=="🔴 FAILED BREAKOUT": rr.append("resistance rejected; price materially below level")
...
def perform_live_refresh(state, mapping, cfg):
    """Fast path: reuse the EXACT full-scan structure snapshot and refresh only live data.

    The previous version rebuilt the structure pool during refresh. V30 deliberately
    stores and reuses the full-scan pool so a refresh cannot silently lose the
    stocks that were eligible in the immediately preceding full scan. Current
    live conditions still decide whether an event remains active.
    """
    frames=state["frames"]; nifty=state["nifty"]
    pool_df=state.get("pool_df",pd.DataFrame())
    if pool_df.empty:
    ...
    if strict_df.empty or "LTP" not in strict_df.columns:
        ready=pd.DataFrame(columns=pool_df.columns); near=pd.DataFrame(columns=pool_df.columns); triggered=pd.DataFrame(columns=pool_df.columns)
    else:
    ...
    # Event candidates also receive historical validation when refreshed.
    ...
    out=dict(state)
    out.update(ready=ready,near=near,triggered=triggered,event=event_df,pool_df=pool_df,live_errors=day_errors+minute_errors,live_map=live_map,run_time=now_ist())
    return out
...
def main():
    ...
    with tabs[0]:
        ...
        st.caption(f"LIVE EVENT RADAR: ⚡ {int(ev_counts.get('⚡ BREAKOUT NOW',0))} now • 🟢 {int(ev_counts.get('🟢 BREAKOUT CONFIRMING',0))} confirming • 🔄 {int(ev_counts.get('🔄 BREAKOUT RECLAIMING',0))} reclaiming • 🚨 {int(ev_counts.get('🚨 BREAKOUT TESTED',0))} tested • 🔵 {int(ev_counts.get('🔵 EXTENDED BREAKOUT',0))} extended • 🔴 {int(ev_counts.get('🔴 FAILED BREAKOUT',0))} failed")
